...
| `silver_tally.cells` (a rendered silver row) | `disagreement_ledger.silver_tally` + `commands/trust.py --silver` + MCP `trust(silver:true)` | **data-directive** (a per-model win-rate a user may act on) | Opt-in surfaces only (council `council_a5ba36c437d492f9`: default output carries at most a pointer, and only once a cell has actually cleared; the launchpad card stays gold-only; MCP returns silver as a SEPARATE key, never merged into `tally`). Gate: a cell renders ONLY at n >= `SILVER_MIN_N` (25 — deliberately a SEPARATE, higher floor than gold's `MIN_TALLY_N`, because the adjudicator is ~11% nondeterministic and n≈10 admits cells one re-run could flip); below-floor cells are COUNTED as withheld, never shown. Crediting is claim-SIDE, never member-vs-council-winner (the member rule was falsified by an 89%-vs-23% opponent-pool artifact inside one family). Unparseable/`unresolved` resolutions are skipped and counted, identity-unknown members mint no cell, and the calibration stamp NAMES ITS SOURCE — the 63% transfer belongs to the re-chaired research corpus, and the text says the live labels are not yet calibrated (re-measure at n>=150 or 2026-11-01). Degenerate tests in `test_silver_tally.py` (below-floor withheld, unparseable-skipped, identity-unknown, empty-home abstain); mutation-proven (floor→1 reds the withheld test; dropping the family fold reds the fold test; editing the stamp reds the provenance test) |
| `core_gate.propose_core` (a core.md write) | `distill.write_core` -> `~/.trinity/core.md`, read first by every chairman | **decision-directive** (the paragraph that conditions every council synthesis) | Was an unconditional overwrite whose only content check was non-emptiness (amd_0153). Now a candidate is a PROPOSAL: it replaces the incumbent only if it prices held-out FOUNDER utterances at no more bits (`TOLERANCE_BITS=0` — ties admit, degradations are refused and the core stays frozen). FAILS CLOSED — a present-but-thin sample (`MIN_HELDOUT=40`) REFUSES and keeps the incumbent; an ABSENT corpus is a cold install where nothing was earned, so it admits. Every candidate is archived to `~/.trinity/core_history/` whatever the verdict, so a rejection is recoverable (the prior behaviour kept one file and no history). Target is prompt nodes ONLY, never council outcomes, so the transcripts-only founder-lock holds; the module reads no env flag and does not touch `TRINITY_REGRESSION_GATE`. Degenerate tests in `test_core_gate.py` (worse-candidate refused, thin-sample refused, empty refused, rejected-still-archived); mutation-proven (bypassing the admit branch reds; `TOLERANCE_BITS`->1e9 admits a degradation; `MIN_HELDOUT`->0 bypasses the thin refusal) |
