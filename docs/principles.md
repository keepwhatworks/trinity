...
The information-theory reading of *why* the loop is the moat: the bits that carry information are the surprising ones. Compression is prediction, so the best representation is the one found by predicting and then studying the **residual** — the failure, the disagreement, the correction — never by re-encoding the consensus you already had. (This is not forecasting the future; the future is designed, not predicted. It is how the *designed mechanism* learns its own symbols.) Trinity's symbols are built from residuals: the taste lens is a correction vector (what you kept minus what you rejected — `me/correction_lens.py`), the trust ledger is the cross-model disagreement, and every green-gate is a hunt for the failure a passing metric hid. A pipeline that only re-encodes agreement is spending its bits where there is no information.

**The loop, end to end — and where it is honest about the gaps.** predict → keep the residual → mine residuals that recur across *independent* contexts (`basins_spanned` in `me/lens_registry.py`) → promote a recurring exception to a symbol (the tension `support_count ≥ ACTIVE_MIN` gate; arcs at ≥3, "2 is coincidence, 3 is a pattern") → settle every bet on held-out reality (the disagreement ledger reads your later work; the palate registry scores prospectively). Three steps are only PARTIAL and are named here as **roadmap, never claimed shipped** (principle #1): (a) eviction is *recency*-based — a tension fades after `RECENCY_DAYS` — not yet *rent*-based (a symbol is not yet evicted for ceasing to reduce residual across contexts); (b) attention aims at the *high*-residual live edge (low-margin basins), not yet at the *fastest-improving* compression (the learning-progress derivative is not computed); (c) confident council disagreements are tallied (the trust ledger) but do not yet set an explicit exploration budget, and council votes are not yet calibration-weighted by each model's track record. Closing (a)-(c) is the compression-loop roadmap; each needs its own pre-registered falsifier before it ships.
...
- **Enforce (dev):** Hide machinery (embeddings, token plumbing, parameter counts) from the default view, but **always** surface the decision seams (which provider won, where they disagreed, the margin). A surface that hides the seam, presenting a verdict as uncontested, is as wrong as one dumping raw machinery. Council surfaces show the split and the winner in human brand terms, not raw slugs.
