...
`src/trinity_local/mcp_server.py` exposes these tools to any MCP-compatible harness (Claude Code, Codex CLI, Antigravity, Cursor). Canonical 3. v1.5 adds two: `ask` for cheap single-call routing, `get_picks` for agent-facing introspection. The in-protocol provider loop adds `import_provider_memory`. The lens-lift adds `lens_generators`. The in-session eval adds `run_eval`. The choice oracle adds `choose` (rank options on the frozen palate direction, live accuracy in every answer). That makes 9 total. Plus MCP Resources at session handshake exposing the lens hierarchy + scoreboards as readable URIs (`trinity://memories/lens.md` etc.). (`get_eval_summary` shipped post-#122 then retired 2026-05-18 in commit `6a7f176`. `record_outcome` retired 2026-05-21 per "we are sunsetting user ratings". Chairman's pick is the supervision signal now. `handoff` retired 2026-05-26 after 0 production usage. Lens flows via MCP Resources. `mark_pick_wrong` retired 2026-06-05. The user-pick/veto layer was removed. The chairman's pick stands alone. `route` removed 2026-06-08 in the loop-primitive surface cut. `ask(mode="route")` is the one routing entrypoint.)
