...
`src/trinity_local/main.py` is a thin dispatcher. Command modules live under `commands/`. Selected commands shown below. The complete live surface (<!-- canonical:cli_command_count -->60<!-- /canonical --> subcommands across <!-- canonical:command_module_count -->25<!-- /canonical --> command modules) is enumerated by `trinity-local --help`.
...
`src/trinity_local/mcp_server.py` exposes these tools to any MCP-compatible harness (Claude Code, Codex CLI, Antigravity, Cursor). Canonical 3. v1.5 adds `ask` for cheap single-call routing (`get_picks` removed 2026-08-11 with the lens-basin router it introspected). The in-protocol provider loop adds `import_provider_memory`. The disagreement ledger adds `trust` (which model you side with when the labs split + recurring cross-provider disagreements; retrieval read-only + LLM-free, tally via `trust --build`). That makes 7 total. (The eval-harness / palate / generators tools — `run_eval` / `choose` / `lens_generators` — were soft-demoted off the surface 2026-07-18; their CLI verbs + engines stay.) Plus MCP Resources at session handshake exposing the lens hierarchy + scoreboards as readable URIs (`trinity://memories/lens.md` etc.). (`get_eval_summary` shipped post-#122 then retired 2026-05-18 in commit `6a7f176`. `record_outcome` retired 2026-05-21 per "we are sunsetting user ratings". Chairman's pick is the supervision signal now. `handoff` retired 2026-05-26 after 0 production usage. Lens flows via MCP Resources. `mark_pick_wrong` retired 2026-06-05. The user-pick/veto layer was removed. The chairman's pick stands alone. `route` removed 2026-06-08 in the loop-primitive surface cut. `ask(mode="route")` is the one routing entrypoint.)
...
5. *(retired)* `get_picks` was the v1.5 introspection tool for the per-lens-basin routing tally. Removed 2026-08-11 with the router it read (council_8817ca0c57a2e4ff; hq_062 replay: fired 58.6%, 42.9% vs a constant's 37.0%, chance-level). A tool that reads a routing table cannot outlive the routing table.
...
- **`global_benchmarks.py` separate path**: read-time blend shipped per Tier 2 #7 (task #52, 2026-05-12). The file structure stayed split (no merge into one module) but `ranker/chairman_picker.py:_blended_pick` reads both `global_benchmarks` + `personal_routing_table` and sigmoid-blends them at decision time. It is the only consumer that matters. The structural separation has zero behavioral cost.
...
7. **MCP tool surface (canonical 3 + `ask` + `import_provider_memory` + `trust` + `verify`).** Canonical: `run_council` (subsumes `judge`), `get_persona`, `get_council_status`. v1.5: `ask` (cheap single-call default. `mode="route"` is the one routing entrypoint after the standalone `route` tool was cut). `get_picks` removed 2026-08-11 with the router it read. In-protocol provider loop: `import_provider_memory` (the agent pipes its own extracted signals into Trinity). Disagreement ledger: `trust` (which model you side with when the labs split + recurring cross-provider disagreements; retrieval read-only + LLM-free, per-model tally gated on K3-band/K4, built via `trust --build` session sampling). 7 total. (The eval-harness / palate / generators tools — `run_eval` / `choose` / `lens_generators` — soft-demoted off the surface 2026-07-18; CLI verbs + engines stay.) Plus MCP Resources at session handshake. (`get_eval_summary` shipped post-#122 then retired 2026-05-18 in the simplification pass, commit `6a7f176`. `record_outcome` retired 2026-05-21 alongside the rest of the rating UX. Chairman's pick is the supervision signal now. `handoff` retired 2026-05-26 after 0 production usage. Lens flows via MCP Resources. `mark_pick_wrong` retired 2026-06-05. The user-pick/veto layer was removed. The chairman's pick stands alone.)
