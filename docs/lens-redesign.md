...
from the layer that didn't need it (moves, where the chairman derives
those at inference time) when the layer that genuinely needs it is **the
lens itself**.
...
1. **Lens accumulation core** (load-bearing). Add the tension registry
   ...
   supporting_evidence_ids`) plus the Stage 4.5 reconcile step (cosine-match
...
stateful counters. That is both simpler than the v1 design and immune
...
- **One idea.** "The lens is a stable, self-owned, evidence-backed
...
- **One primitive, three jobs.** Embedding cosine does basin-membership
  ...
  vocabulary canonicalization (4.25). The kernel applied at every
  layer. The recursion isn't decoration, it's the same operation reused. That
