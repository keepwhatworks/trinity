...
def render_unified_council_page(bundle: PromptBundle, outcome: CouncilOutcome) -> str:
    ...
    <article class="card answer-card">
      ...
      <h3>{_esc(provider_model_brand(provider))}</h3>
    ...
    chairman_provider_label = provider_model_brand(outcome.primary_provider) or "unknown"
    ...
    # A council needs at least TWO DISTINCT voices to have a contest. A SOLO
    # council (1 responder) — OR a council whose members are all the SAME
    # provider (e.g. claude·claude·claude — the chairman's winner/runner_up all
    # key on that single slug, so the card reads "Claude · runner-up: Claude")
    # — has no contest, so suppress the winner/agreed/disagreed competition
    # framing in the routing label (the share-card twin, council_card.py).
    # Gate on the count of DISTINCT provider slugs, not the raw member count:
    # the same-provider roster was the unfixed sibling of the 1-responder solo
    # branch (the live page already collapses it via its provider-keyed members
    # map).
    _solo = len({m.provider for m in outcome.member_results}) <= 1
    routing_label_html = _render_routing_label_section(outcome, solo=_solo)

    # Honest partial-council disclosure (the #238 honest-degradation lineage,
    # mirroring the LIVE page's failedMembersFor note). The runner records
    # casualties in metadata.failed_members but EXCLUDES them from
    # member_results, so a 2-of-3 council — e.g. a rate-limited premium provider
    # (GPT-5.5 hitting its quota) — would otherwise render on this PERSISTENT,
    # shareable review artifact identically to a deliberate 2-model council,
    # hiding that a 3rd was attempted and failed. Count-only (no provider names)
    # to stay clear of the #275 slug-vs-brand display call, same as the live page.
    _failed = (getattr(outcome, "metadata", None) or {}).get("failed_members")
    _failed_count = len(_failed) if isinstance(_failed, list) else 0
    _responded_count = len(outcome.member_results)
    failed_disclosure_html = ""
    if _failed_count > 0 and _responded_count > 0:
        _plural = "" if _failed_count == 1 else "s"
        _was_were = "was" if _failed_count == 1 else "were"
        failed_disclosure_html = (
            f'<p class="meta status-error">⚠ {_failed_count} provider{_plural} '
            f"attempted but failed and {_was_were} excluded — this synthesis is "
            f"over the {_responded_count} that responded.</p>"
        )
    elif _failed_count > 0 and _responded_count == 0:
        # Total failure: NO member responded, so there is no synthesis. The old
        # "this synthesis is over the 0 that responded" copy was a flat
        # self-contradiction — it asserted a synthesis exists while saying nobody
        # answered. (#238 honest-degradation lineage; the live-page twin shares
        # this two-branch split.)
        failed_disclosure_html = (
            '<p class="meta status-error">⚠ Every provider attempted but failed '
            "to respond — there's no synthesis to show.</p>"
        )
    ...
    # A corrupt outcome can carry round_number as a non-int-coercible value
    # (e.g. the string "abc"); `int(round_number)` below (in pageData) raised
    # ValueError ("invalid literal for int() with base 10") — a 500 on the
    # unified review page. Coerce to a safe int, defaulting to round 1.
    try:
  ...
  <style>
    ...
    /* The answer card is a READ-ONLY artifact on this static, shareable review
       page — the click-to-pick UI was retired 2026-05-22 (the chairman's pick is
       the sole supervision signal now; see render_unified_council_page's
       docstring). The <article> carries no @click / tabindex / role / keyboard
       handler and CouncilApp never mutates a selection, so it must NOT advertise
       interactivity: `cursor: pointer` + a hover LIFT toward var(--action) (the
       CTA color) + a :focus-visible ring + a .selected rule were all VESTIGIAL
       affordance-lies left over from that retired pick UI — clicking a card did
       nothing (DOM unchanged, no dispatch, never focusable). Keep min-width:0
       (load-bearing for the overflow containment below) and a SUBTLE, non-CTA
       hover (a faint border-tint, no lift, default cursor) that reads as "a card"
       not "a button". */
    .answer-card {{
      transition: border-color 0.2s ease, box-shadow 0.2s ease;
      /* Grid items default to min-width:auto, which refuses to shrink below the
         min-content size of their contents. A member answer with a wide code
         block (no-wrap <pre>) or a long unbreakable token then stretches the
         card PAST the viewport — horizontal scroll on the review-link companion
         page people open on phones (a coding council's answers are full of
         code). min-width:0 lets the card shrink to its 1fr track; the
         containment rules below keep wide content scrolling INSIDE the card
    ...
    /* Wide markdown content (code blocks, tables, long unbreakable tokens) in a
       member answer OR the synthesis must scroll/wrap WITHIN its container, not
       stretch the page on a phone. Pairs with .answer-card min-width:0 above. */
    .markdown-body {{
      min-width: 0;
    ...
    /* The routing-label card (winner / scores / structured agreed+disagreed
       claims) renders corpus text — a claim or why_matters can carry a long
       unbreakable token (an identifier / URL), which overflowed the 375px
       review-link companion by ~490px (found 2026-06-07 browser-probing the new
       claims section). Break long tokens so they wrap inside the card. */
    .routing-label-card {{
      min-width: 0;
      overflow-wrap: break-word;
      /* Cap the verdict prose at a comfortable reading measure on wide screens.
         The answer-card GRID below stays full width — only this prose card is
         constrained. Left-aligned to match the page's stacked card flow. */
      max-width: 720px;
    }}

    .synthesis-section {{
      margin-bottom: 32px;
      /* Same comfortable reading measure as the verdict prose above; the
         answer-card grid is a separate full-width section and is untouched. */
      max-width: 720px;
    }}
    ...
    .rank-badge {{
      ...
      padding: 4px 12px;
      border-radius: 999px;
      /* --action (#3f777c, white 5.07:1 AA), NOT --success (the #4f9095 WORDMARK
         teal): white "Lens pick" on --success was 3.65:1 — below the AA body
         floor. --success/--accent is the WCAG-exempt brand mark, not a white-text
         fill; --action is the purpose-built AA-readable teal the CTA already uses.
         (The teal→white-text sub-AA fill class — sibling of the Iter-198 .done
         badge + the CTA-button AA push.) UX sweep 2026-06-21. */
  ...
  <div id="council-app" v-scope="CouncilApp(pageData)">
    <header class="trinity-topbar">
      ...
      <!-- Chrome label, NOT the page's primary heading — the council QUESTION
           (the <h1> in <main> below) is the page title. This banner label was an
           <h1>, so it competed with the question h1 for the sole-h1 slot a screen
           reader navigates to (WCAG 2.4.6 / 1.3.1). Demoted to a <p>; the visual
           is unchanged (the .topbar-title CSS keys on the class, not the tag). -->
      <p class="topbar-title">Council review</p>
    </header>
    <main>
      ...
      <section class="card chain-actions">
        ...
        <div v-if="chainError"
             style="margin: 12px 0 16px; padding: 12px 16px; border-left: 3px solid #4f9095; background: rgba(79, 144, 149, 0.08); color: #714824; font-size: 13px; border-radius: 4px;"
             role="alert">
          <strong style="display: block; margin-bottom: 4px;">Could not start next round</strong>
          <span style="display: block;">{{{{ chainError }}}}</span>
          <a href="#" @click.prevent="chainError = ''" style="display: inline-block; margin-top: 6px; color: #34666b; font-size: 12px;">Dismiss</a>
        </div>
        ...
        <div class="chain-refine-row" v-if="!chainBusy">
          <textarea
            ...
            placeholder="Or refine with a new directive… (⌘/Ctrl+Enter to send)"
  ...
  <script>
    ...
    // #275 model-brand display for the CLIENT-side chain poller. The static
    // winner/members on this page are branded SERVER-side (provider_model_brand),
    // but the live chain-action status detail ("<member> is responding…") is
    // painted in JS from the polled status file, whose active_provider is the raw
    // CLI/capture slug. Without a JS brander it leaked "antigravity is responding…"
    // / "codex is responding…" — the #275 raw-slug straggler the LiveCouncilApp
    // sibling poller already avoids via its own formatProviderLabel. This is the
    // ONE council page that lacked the helper (launchpad + LiveCouncilApp define
    // their own in-scope copy), so define a local copy here rather than risk a
    // redeclaration by hoisting it into the shared runtime. Mirrors
    // council_schema._LEGACY_PROVIDER_ALIASES + _MODEL_BRAND_DISPLAY.
    ...
    function formatProviderLabel(provider) {{
      ...
      const labels = {{ claude: 'Claude', antigravity: 'Gemini', codex: 'GPT', mlx: 'MLX', openai: 'GPT' }};
      ...
      return {{
        ...
        _startChainAction(actionName, additionalArgs, heading, detail) {{
          ...
          const extensionAction = {{
            ...
            // Underscore key — capture_host's ACTION_ALLOWLIST reads
            // payload.get('status_token'). A hyphen here (the CLI FLAG
            // spelling) silently dropped the token, so council-iterate ran
            // under the bundle_id, not the chain token the page polls →
            // "council never started / no status file" (founder 2026-06-12).
            // The launchpad dispatch has always used underscore; this matches.
        ...
        _pollChainStatus(statusToken) {{
          ...
          // Mirror startPolling's MAX_MISSING_POLLS give-up. A chain action
          // (Continue / Refine) whose status file never appears — dispatch
          // failed silently, the COMMON cause being the Chrome extension isn't
          // installed in this browser — must NOT poll forever. startPolling got
          // this guard for the load path (the launch_mpm0bght_gx1y9v symptom:
          // "sat polling … Council running forever with zero feedback"); the
          // chain-action poller was the un-fixed sibling instance.
            ...
            loadStatusScript(statusToken, (status) => {{
              if (!status) {{
                ...
                if (missingPollCount >= MAX_MISSING_POLLS) {{
                  ...
                  this.chainBusy = false;
                  this.chainError = "This round never started — the launch URL " +
                    "was built but no status file was ever written, so dispatch " +
                    "likely failed silently (common cause: the Chrome extension " +
                    "isn't installed in this browser). Reload it at " +
                    "chrome://extensions and click Continue again.";
                }}
                return;
              }}
              missingPollCount = 0;  // status showed up — dispatch worked; reset.
              if (status.status === 'running') {{
                ...
                if (synthesisStatus === 'running') {{
                  ...
                  this.chainStatusDetail = `${{formatProviderLabel(status.active_provider)}} is responding…`;
                }}
                return;
              }}
              if (status.status === 'completed' && status.review_path) {{
                clearInterval(this._chainPollHandle);
                this._chainPollHandle = null;
                // Pass council_id so the SANDBOX (side panel) hydrates the verdict
                // instead of landing on a blank token-less ./live_council.html
                // (the navigate-to-nowhere strand, driven 2026-06-18).
