...
def render_launchpad_html(*, page_data: dict, recent_sidebar: str = "", title: str = "Trinity · Ask all three", view: str = "home") -> str:
  ...
  <style>
    ...
    /* .rail-council-splits is the decision chip ("3 splits · 2 settled" /
       "unanimous") — same pill as the rounds badge so the meta line stays one
       visual family; it differs only in what it says, not how it looks. */
    .rail-council-rounds, .rail-council-splits {{
  ...
  <main>
    <div class="launchpad-shell lp-view-{view}" id="launchpad-app" v-scope="LaunchpadApp(pageData)" v-cloak @vue:mounted="init">
      ...
      <!-- The trust card renders on BOTH views (untagged — neither home-card nor
     stats-card) as of 2026-08-03: it is the product's one behaviourally
     MEASURED claim ("which model you side with when the labs split"), and the
     view-class split had left the home page with zero trace of it — the hero
     lived only behind the stats link. v-if="trustData" keeps a cold install's
     home clean (the card self-hides until a ledger exists). -->
<section class="card" v-if="trustData">
