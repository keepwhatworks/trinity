...
## Priority 4 — Distribution & registries

Work that gets Trinity in front of more polyharness users:

- **MCP-dropdown distribution** — submit Trinity to the curated
...
- **Cross-provider benchmark publication** — keep the three provider
  eval cards refreshed from matched-coverage runs (`trinity-local
  eval-run --target <provider>` then `eval-share`), so the leaderboard
  headline is anchored to current, equal-N data for all three models.
...
## Priority 5 — Trajectory work

Concrete items worth contributing on:
...
- **`plan_and_execute` MCP tool** — fully sunset 2026-05-22; the
  ...
  preserved in `docs/historical/retirement-log.md`.
...
silently disable real-corpus checks downstream (see `docs/historical/principles.md`
meta-principle #19 for the incident this rule came from). Use `monkeypatch.setenv` in an
...
```bash
git clone https://github.com/keepwhatworks/trinity trinity-local
cd trinity-local
...
.venv/bin/python -m pytest -q       # the test suite, ~140s; gate must stay green
python scripts/browser_smoke.py     # <!-- canonical:smoke_surface_count -->35<!-- /canonical -->-surface UI verification (Playwright)
```
...
5. **HF Hub offline by default.** `main()` pins `HF_HUB_OFFLINE=1` + `TRANSFORMERS_OFFLINE=1`
   via `setdefault` at startup. The embedding model is pulled once via
   `trinity-local download-embedder` (or `HF_HUB_OFFLINE=0 huggingface-cli download nomic-ai/modernbert-embed-base`);
   after that Trinity loads from `~/.cache/huggingface/hub/` and never contacts the Hub during normal operation.
