...
from .design_system import DIE_MARK_INLINE_SVG, page_data_script_json, render_html_footer, render_html_head
...
def render_launchpad_html(*, page_data: dict, recent_sidebar: str = "", title: str = "Trinity · Ask all three", view: str = "home") -> str:
    # Chart.js is ~205 KB and ONLY drives the /stats charts (ELO + provider win-
    # rates). The fusion-first HOME has no chart, so we no longer load it eagerly
    # in <head> (parsing 205 KB on every side-panel open lingered the loading
    # spinner, founder-caught 2026-06-16). renderChart() lazy-injects it from
    # CHART_JS_SRC only when a stats view actually renders a chart.
    head = render_html_head(title, extra_head="")
    footer = render_html_footer()
    # On the dedicated /stats page the browser-capture / import-export cards are
    # the page's content, so pop their <details> open — analytics shouldn't be
    # buried behind a collapsed disclosure there. On home the whole
    # .demoted-card-wrapper is display:none (the CSS rule above — the inner cards
    # are stats-only AND the wrapper's summary, which would otherwise leak a
    # dead "expand" toggle), so the attribute is a no-op.
    details_open = " open" if view == "stats" else ""

    return f"""{head}
  <style>
    /* Launchpad view-class split (openrouter.ai/fusion-style "one clear action"
       home + a separate /stats page). The HTML string ALWAYS contains every
       section — the page only toggles VISIBILITY via the body view class, so
       string-presence tests stay green. home = the cross-provider council +
       lens upsell; stats = all analytics/diagnostics. settings-modal + the
       council rail are untagged, so they show on BOTH views. */
    .lp-view-home .stats-card {{ display: none !important; }}
    .lp-view-stats .home-card {{ display: none !important; }}
    /* The memory-health card is an analytics/diagnostics card (stats-only), but
       a structural guard test anchors on its exact section-opener class string,
       so we can't append the stats-card token to its class attribute. Hide it on
       home via the .memory-health-card hook instead — same effect as a stats-card
       tag. (The dispatch-banner card reuses .memory-health-card for styling and
       is also stats-only, so this correctly hides both on home.) */
    .lp-view-home .memory-health-card {{ display: none !important; }}
    /* The browser-capture / import-export <details> WRAPPER carries no stats-card
       token (only the cards INSIDE it do), so on home the inner cards were
       display:none but the wrapper's <summary> ("Browser capture … · expand")
       stayed visible — a DEAD DISCLOSURE: tapping "expand" on home revealed
       nothing (every non-stale child is stats-only). The wrapper now gets the
       `stats-card` token CONDITIONALLY (only when NOT stale — see the <details>
       :class), so the healthy/empty disclosure is fully hidden on home rather
       than leaking a summary that expands to an empty panel. The STALE state
       (silent capture breakage, the #147 self-heal CTA) deliberately stays
       reachable on home (UX iter 82) — there the wrapper keeps no stats-card
       token. /stats keeps the wrapper in all states (force-open, cards visible). */
    .launchpad-shell {{
      display: grid;
      /* minmax(0, 1fr), NOT the default implicit column. The implicit column's
         `min-width: auto` sized to the widest card's min-content (~982px) and
         refused to shrink, so the launchpad scrolled sideways on anything
         narrower than ~1280px — a phone (~625px over), a tablet, split-screen, a
         small laptop (the whole 769-1280px range below the rail-sidebar width).
         v1.7.182 scoped the fix to max-width:768px, which left that tablet/laptop
         dead zone overflowing. Letting the single column shrink to its container
         reflows the cards at EVERY width; on a wide desktop they just fill to
         `main`'s max-width (the intended content width anyway). */
      grid-template-columns: minmax(0, 1fr);
      gap: 32px;
    }}
    /* Simple, google.com-style HOME: collapse the card grid into ONE centered
       focal column (max 720px) — brand mark, headline, the council composer (the
       only hero), then a couple of quiet cards. /stats (lp-view-stats) keeps the
       full-width analytics grid. The composer is pulled directly under the hero
       via flex order so a first-time eye lands on #council-prompt. */
    .lp-view-home {{
      max-width: var(--composer-max, 720px);
      margin: 0 auto;
      display: flex;
      flex-direction: column;
      gap: 18px;
    }}
    .lp-view-home > * {{ order: 3; }}
    .lp-view-home > .hero-shell {{ order: 0; }}
    .lp-view-home > .launch-grid {{ order: 1; }}
    .lp-view-home > .hero-proof {{ order: 2; }}
    .lp-view-home .hero-shell {{
      border: none;
      box-shadow: none;
      background: none;
      padding: clamp(3vh, 6vh, 60px) 0 0;
      text-align: center;
      position: relative;
    }}
    .lp-view-home .hero-shell > div {{ width: 100%; }}
    .lp-view-home .hero-shell .eyebrow {{
      display: inline-flex; gap: 6px; align-items: center; justify-content: center;
    }}
    .lp-view-home .hero-shell h1 {{ font-size: clamp(27px, 4.5vw, 34px); line-height: 1.1; margin: 10px 0 6px; }}
    .lp-view-home .hero-shell .lede {{ font-size: 17px; }}
    /* The settings gear floats top-right of the centered hero. */
    .lp-view-home .hero-shell > button {{ position: absolute; top: 6px; right: 0; }}
    /* The composer is the focal element — soften its corners. */
    .lp-view-home .launch-grid .card {{ border-radius: 20px; }}
    @media (max-width: 560px) {{
      .lp-view-home .hero-shell {{ padding-top: 3vh; }}
      .lp-view-home .hero-shell h1 {{ font-size: 25px; }}
    }}
    /* The routing / cortex cheat-sheet tables have a fixed multi-column
       min-content (~932px) no column trick can shrink — scroll each inside its
       own card instead of widening the page. Unconditional: a no-op when the
       table fits (desktop), engages the moment the card is narrower (any
       tablet/phone width), so there's no breakpoint dead zone. */
    .routing-table {{ display: block; overflow-x: auto; }}

    /* Cheat-sheet commands (md-code-block) are single-line CLI snippets wider
       than a phone card — `trinity-local council --task "<your question>"` is
       ~398px vs a ~290px card. The page clips overflow-x, so without wrapping
       the right half of the command is hidden on mobile (verified at 375px: the
       code ran to x=453, 78px past the viewport, with no scroll). Wrap them so
       the FULL command stays visible + copyable; overflow-wrap breaks an
       unbreakable token (e.g. the curl|bash install URL) rather than overflow.
       max-width + overflow-x:auto are the belt-and-suspenders fallback. No-op on
       desktop where the command fits on one line. Same unconditional approach as
       .routing-table above. */
    .md-code-block {{ max-width: 100%; overflow-x: auto; }}
    .md-code-block code {{ white-space: pre-wrap; overflow-wrap: anywhere; }}
    ...
    textarea:focus {{
      ...
      box-shadow: 0 0 0 3px rgba(79, 144, 149, 0.1);
    }}
    ...
    .suggestion-chip {{
      ...
      color: var(--accent-deep);  /* AA 5.1:1 on the teal tint (was --action 4.0:1) */
      border: 1px solid rgba(79, 144, 149, 0.18);
    }}

    /* Click-to-copy affordance for the PRIMARY command code blocks (council /
       lens / extension install). The block keeps its <pre> look; a small
       "Copy" → "✓ Copied" button sits top-right (revealed on hover, always
       reachable by keyboard). Makes the cold-start commands one-tap-copyable
       like the eval + setup chips — the first-run actions shouldn't be harder
       to copy than the secondary ones. */
    .code-copy-wrap {{
      position: relative;
    }}
    .code-copy-wrap .copy-badge {{
      position: absolute;
      top: 6px;
      right: 8px;
      font: 600 11px ui-monospace, SFMono-Regular, monospace;
      letter-spacing: 0.03em;
      color: var(--accent-deep);  /* AA 5.4:1 on the teal-tint chip (was --action 4.3:1) */
      background: rgba(79, 144, 149, 0.12);
      border: 1px solid rgba(79, 144, 149, 0.22);
      padding: 2px 8px;
      border-radius: 5px;
      cursor: pointer;
      opacity: 0.7;
    ...
    /* Touch-target floor (WCAG 2.5.5 / Apple HIG): the visible pill is ~19px
       tall (compact by design — it's a corner overlay on a code block), HALF
       the 44px floor on the touch-width side panel where the cold taste card's
       "trinity-local lens" copy badge is the brand-new user's PRIMARY
       lens-build affordance. Extend the HIT AREA to 44x44 with a transparent
       pseudo-element centered on the pill so the click box clears 44px WITHOUT
       bloating the visible chip or overlapping the code text (the same hit-area
       pattern the sidepanel tip x + live-council Quote button use). Founder
       symptom: "every action button clear 44px on touch widths". */
    .code-copy-wrap .copy-badge::before {{
      content: "";
      position: absolute;
      top: 50%;
      left: 50%;
    ...
    .code-copy-wrap:hover .copy-badge {{
      opacity: 1;
    }}
    .code-copy-wrap .copy-badge:focus-visible {{
    ...
    /* Cross-memory chip — shared base for the small bordered pill links
       ...
       of the user-rating UX; the in-page card grid itself was removed
       2026-06-06 — see browser_smoke Surface 25 retirement guard.) */
    .cross-memory-chip {{
      font-size: 10px;
    ...
    .cross-memory-chip:hover {{
      color: var(--action);
      background: rgba(79, 144, 149, 0.06);
    }}
    ...
    /* The taste-card "Spans" chips now carry the basin's TOP TERMS (prose,
       e.g. "design · arch") instead of a 3-char id, so a long 3-term label
       must wrap INSIDE the chip + cap its width — otherwise a single nowrap
       chip overflows the narrow side panel at 320px. */
    .lens-basin-chip {{
      white-space: normal;
      max-width: 100%;
    ...
    /* Tick #79: ↻ Rebuild chip shared by the lens-rebuild (tick #76)
       ...
       memory_viewer.py) — same '↻ Rebuild' copy on a populated memory
       (unified in tick #79). The launchpad chips only render on BUILT data
       (card eyebrows gated on populated state), so they always read
       "↻ Rebuild"; the viewer flips to "↻ Build" on a not-built-yet file
       (UX sweep iter 73) — these chips live in different CSS contexts. */
    .lp-rebuild-chip {{
      font-family: ui-monospace, monospace;
      font-size: 11px;
    ...
    .taste-block-label {{
      ...
      text-transform: uppercase;
      /* AA: --accent-deep #34666b (5.0:1 on --surface). NOT rgba(79,144,149,0.7)
         — the WORDMARK teal at 0.7 alpha was 2.3:1 (the wordmark-teal-as-text /
         alpha-dimmed-text class). The wordmark teal is the fill, never the label. */
      color: var(--accent-deep, #34666b);
      font-weight: 600;
      margin-bottom: 10px;
    }}
    /* Embedder-degraded trust banner — the launchpad twin of the memory
       viewer's .viewer-trust-banner. Fires when lens_health._embedding_backend
       is DEGRADED (the SHA-1 TF-IDF fallback) so the confident taste card SAYS
       the displayed tensions are caricatures instead of painting them as
       settled truth (#35 green-while-degraded). Terracotta left border +
       deepened-danger label (AA on the 0.10 tint), body text-primary 10.5:1. */
    .lp-trust-banner {{
      display: flex;
      align-items: flex-start;
      gap: 10px;
    ...
    /* "Justified by · click to see the work" backref expand (the README
       "show its work" pledge). The privileged / sacrificed / verbatim text
       is USER-CONTROLLED corpus content (sourced from preference_acts.jsonl,
       derived from the user's own prompts) — so it can carry a long
       UNBREAKABLE token (a ~/.cache/... path, a URL, a base64 blob, a
       stack-trace line with no spaces). The expand block's `max-width: 600px`
       inline style does NOT constrain such a token: without overflow-wrap the
       token demands its full intrinsic width, blowing the whole launchpad past
       a 320px side panel (docOverflow 714 on a real-panel drive — horizontal
       scroll + the privileged token + pa_NNN summary run off the right edge,
       precisely when the traceability feature is meant to shine). Force the
       chip + every text descendant to break anywhere and allow the flex/block
       boxes to shrink below their content's intrinsic min-width. min-width:0 on
       the <details> itself is the load-bearing belt — a flex child defaults to
       min-width:min-content, which a long token pins wide. */
    .lens-decision-chip,
    .lens-decision-chip * {{
      min-width: 0;
      overflow-wrap: anywhere;
    ...
    .taste-list li {{
      ...
      border-left: 3px solid rgba(79, 144, 149, 0.35);
      border-radius: 0 8px 8px 0;
      line-height: 1.5;
      /* Every taste-list item renders LLM/corpus prose — a paired-lens
         pole/failure mode, a combined-lens why_matters, or a VERBATIM abstract
         lens statement (.taste-list-quotes) lifted straight from the user's
         lens.md. That free text is exactly where a long unbreakable token shows
         up (a dev identifier / file path / URL / config key the user keeps
         citing), and without a break rule a single such token demands its full
         intrinsic width and pins the whole launchpad past a 320px side panel
         (docScrollW 610 on a real drive — horizontal scroll + the abstract-lens
         statement runs off the right edge). overflow-wrap inherits to the
         title/why/failure/quote descendants. Same class the design system gives
         h1/h2/h3 + code/pre + the post-hoc review .alert-box (Iter 287/288). */
      overflow-wrap: break-word;
      word-break: break-word;
    }}
    /* The hero-proof card's cold-open line (🪞 "One axis your lens already
       surfaces: …") interpolates the lens tension poles (pole_a/pole_b) VERBATIM
       — the SAME LLM/corpus free text as .taste-list li above, where a long
       unbreakable token (a dev identifier / file path / URL / config key the user
       keeps citing) lives. It carried only an inline style (font/padding) and NO
       break rule, so a long pole token demanded its full intrinsic width and blew
       the HOME view to ~952px at a 320px viewport (documentElement scrollWidth 952
       vs clientWidth 320 — horizontal scroll). This is the long-unbreakable-token
       class, and .cold-open was the asymmetric miss the taste-card census never
       reached because it sits in a SEPARATE card (.hero-proof) with its own inline
       style. .council-value is its structural twin (same card, corpus-derived
       family/leader labels), so break it too. Same rule the design system gives
       h1/h2/h3 + code/pre + the taste-list li + the review .alert-box. */
    .cold-open,
    .council-value {{
      overflow-wrap: break-word;
      word-break: break-word;
    ...
    .taste-vocab-chip {{
      ...
      font-size: 12px;
      color: #333;
      /* The chip body is a VERBATIM phrase the user keeps using (v.phrase from
         vocabulary.md) — frequently a single unbreakable monospace token (a
         function/path/config-key name like render_unified_council_page). In the
         flex-wrap .taste-vocab row a flex child defaults to min-width:min-content,
         so that token pins the chip — and the whole launchpad — wider than a
         320px side panel (scrollW 575, rightEdge 606 on a real drive). Let the
         chip shrink and break the token anywhere (matches .lens-basin-chip /
         .lens-decision-chip, the other monospace corpus chips). Iter 288. */
      min-width: 0;
      max-width: 100%;
      overflow-wrap: anywhere;
      word-break: break-word;
    ...
    /* The endpoint VALUE span ({{ displayedEndpoint }}) is a flex child with the
       flex default min-width:auto, sized to its content's min-content width. A
       real configured GA4/collector endpoint is a long separator-free URL, which
       pins that min-content wide — pushing .setting-row → the .card flex item
       PAST a narrow (side-panel / phone) viewport so the WHOLE settings modal
       clips off both edges (header, ×, body all unreachable at 393/375/360/320).
       min-width:0 lets the value shrink; overflow-wrap/word-break wraps the URL
       inside the row instead of blowing out the card. The label (first child) is
       short — keep it from shrinking/char-breaking so only the value wraps. Same
       flex min-width:auto overflow class fixed at .taste-vocab / the corpus chips
       above. */
    .setting-row > .meta:last-child {{
      min-width: 0;
      overflow-wrap: anywhere;
      word-break: break-word;
    }}
    .setting-row > .meta:first-child {{
      flex-shrink: 0;
    }}
    ...
    .icon-action {{
      /* position:relative so the touch-target ::before below anchors to the
         visible 34px circle. */
      position: relative;
      display: inline-flex;
      align-items: center;
    ...
    /* Touch-target floor (WCAG 2.5.5 / Apple HIG): every .icon-action is a REAL
       action button — Copy install command (tier card + provider-command rows +
       embedder download), Reset anonymous ID, Ingest transcripts — yet the
       visible circle is a compact 34px (HALF-step under the 44px floor) and on a
       touch-width launchpad (393/320) those are the user's only tap targets for
       those actions. Extend the HIT AREA to 44x44 with a transparent centered
       pseudo-element so the click box clears 44px WITHOUT bloating the visible
       circle or shoving siblings in the .provider-command / .setting-value flex
       rows. Same hit-area pattern as the .copy-badge::before above + the
       sidepanel tip x + the live-council Quote button. Founder symptom: "every
       action button clear 44px on touch widths" — the .icon-action class was the
       un-fixed sibling of the Iter-161 copy-badge fix. */
    .icon-action::before {{
      content: "";
      position: absolute;
      top: 50%;
    ...
    .icon-action:hover {{
      ...
      box-shadow: 0 6px 18px rgba(79, 144, 149, 0.12);
    }}

    /* SOLID --action (#3f777c, 4.89:1 on the --surface circle / 4.28:1 on --bg-base),
       NOT the old rgba(79,144,149,0.28) wordmark-teal-at-28%-alpha (composited 1.36:1
       on --surface — a keyboard user Tab-focusing Copy-install / Reset-anonymous-ID /
       Ingest got a near-invisible ring, failing WCAG 2.4.7 Focus Visible + 1.4.11
       Non-text Contrast 3:1). Aligns with the sibling .copy-badge:focus-visible above,
       which already rings solid --action. */
    ...
    .council-card-link:hover .council-card {{
      ...
      border-color: var(--action);
      box-shadow: 0 12px 30px rgba(79, 144, 149, 0.14);
    }}
    ...
    .status-error {{
      color: #8b1e1e;
      /* launchError / operation.error is the raw runner error (status.error =
         str(exc) / why[:200]) — arbitrary output that can carry a separator-free
         path / URL / hash / stack-trace token. Without a break rule that token
         forced the launch-status card (and the document) wider than the
         viewport — the dispatch-failure ribbon streamed off-screen right and the
         320px home horizontal-scrolled to ~1580px. The live-council twin
         (.status-error in council_review.py) + the running card
         (.provider-status-detail) + the popup .panel-tip all break this class;
         this ribbon was the asymmetric miss. */
      overflow-wrap: anywhere;
      min-width: 0;
    }}
    ...
    .status-message {{
      ...
      /* --action-hover #34666b (5.2:1), NOT --action #3f777c: the busy status
         message sits ON the .launch-status teal wash (rgba(79,144,149,0.05)),
         where --action composites to only 4.08:1 — below AA 4.5. Mirrors the live
         council page's .status-message (council_review.py). action-teal-on-wash class. */
    ...
    .provider-status-row {{
      display: grid;
      /* minmax(0, …) on EVERY track (not bare 88px 84px 1fr): a grid track's
         implicit min is `auto` = the cell's min-content, so the fixed name/badge
         columns + the 1fr detail's longest word demand a row min-width that, on a
         320px side panel, pushes the whole launch-status card ~30px past the
         viewport (founder symptom: horizontal scroll on the running council card).
         minmax(0, …) lets the tracks shrink below min-content so the row — and the
         card — stays inside the column. Same belt as .launch-grid's minmax(0,1fr). */
      grid-template-columns: minmax(0, 88px) minmax(0, 84px) minmax(0, 1fr);
      gap: 12px;
      align-items: start;
      font-size: 15px;
      color: var(--text-primary);
    ...
    /* Narrow panel (the side panel can be dragged to ~320px): the third
       `minmax(0,1fr)` detail track collapses to a ~30px sliver that wraps the
       status text ONE CHARACTER PER LINE (founder symptom on the running council
       card). Below 360px, STACK the row — [name][badge] on the top line, the
       detail on its own full-width line below — so the detail reads normally
       instead of as an unreadable vertical strip. The badge already echoes the
       state word, so the stacked detail only ever adds a real sentence. */
    ...
    .provider-status-name {{
      font-weight: 600;
      /* The provider label (Claude / GPT / Gemini) is short, but in the now
         shrinkable minmax(0,88px) track an unexpected long slug must wrap, not
    ...
    .provider-status-badge.running {{
      background: rgba(79, 144, 149, 0.08);
      color: var(--accent-deep);  /* AA on the teal tint */
    ...
    .provider-status-detail {{
      color: var(--text-secondary);
      line-height: 1.4;
      /* A member-error detail can carry a long unbreakable token (a path or URL);
         in the shrunk minmax(0,1fr) track it must wrap, never force the row wider
         than the card (the 320px running-council horizontal-scroll class). */
      overflow-wrap: anywhere;
      min-width: 0;
    }}
    ...
    /* Stop-council / Open-page ghost buttons live on the .launch-status teal wash;
       the global .button.ghost (--action) is 4.08:1 there. Deepen to --action-hover
       so the running-council controls clear AA on the wash (mirrors council_review.py). */
    ...
    /* .recent-filter-chip and its modifiers removed 2026-05-21 —
       ...
       other way to sort?"). Title search + recency-order remain in the
       left rail; the in-page council grid + its search box were removed
       2026-06-06 — chip-row + grid-search CSS no longer reach any DOM. */
    /* Persistent council rail (ChatGPT/claude.ai pattern, #council-sidebar
       2026-06-01): councils were buried in the single-column scroll. A fixed
       left rail keeps them always reachable; main content shifts right on wide
       screens and the rail self-hides on narrow ones so mobile stays single-col. */
    .council-rail {{
      position: fixed; top: 0; left: 0; width: var(--rail-w, 288px); height: 100vh; height: 100dvh;
      overflow-y: auto; box-sizing: border-box; z-index: 58;
      /* OPAQUE — as a floating drawer it sits over the composer, so a near-
         transparent tint (the old rgba 0.045) let the content bleed through it
         (founder-caught in the real side panel). A solid surface also reads as a
         proper nav panel on desktop. */
      background: var(--surface-muted);
      border-right: 1px solid var(--border); padding: 0 14px 40px;
      /* Off-canvas by DEFAULT (the mobile drawer state); desktop re-shows it in
         the >=1024 block below. transform keeps it in layout but translated, so a
         closed rail contributes nothing to scrollWidth (the #291 overflow guard).
         The top clearance for the fixed hamburger now lives in the STICKY
         .rail-header (padding-top: 56px), so the heading + search box stay pinned
         while the council list scrolls under them. */
      transform: translateX(-100%); transition: transform 0.18s ease;
    }}
    @media (prefers-reduced-motion: reduce) {{ .council-rail {{ transition: none; }} }}
    /* Sticky rail header (heading + search) — pins to the top of the scrolling
       rail so the filter is always reachable no matter how far the council list
       is scrolled. The rail's top padding is 0 (it lives here instead): this
       header re-pads 56px to clear the fixed hamburger, bleeds -14px over the
       rail's side padding so its opaque background fully hides rows scrolling
       beneath it, and top: 0 pins it at the scrollport top. */
    .rail-header {{
      position: sticky; top: 0; z-index: 1;
      margin: 0 -14px 10px; padding: 56px 14px 0;
      background: var(--surface-muted);
    }}
    .council-rail h2 {{
    ...
    .rail-council {{
      display: block; padding: 8px 11px; border-radius: 9px; margin-bottom: 1px;
      text-decoration: none; color: var(--text-primary);
    }}
    .rail-council:hover {{ background: rgba(120, 99, 70, 0.09); }}
    .rail-council-title {{
      display: block; font-size: 13px; line-height: 1.3;
      white-space: nowrap; overflow: hidden; text-overflow: ellipsis;
    }}
    .rail-council-meta {{ display: block; font-size: 11px; color: var(--text-secondary); margin-top: 2px; }}
    /* Chain-round count pill — marks a multi-round refine/continue council in the
       rail so it reads distinct from a one-shot. --accent-deep on --accent-soft is
       the AA-pinned small-badge teal pair (palette push-to-AA). */
    .rail-council-rounds {{
      display: inline-block; margin-left: 6px; padding: 0 6px; border-radius: 8px;
      background: var(--accent-soft); color: var(--accent-deep);
      font-size: 10px; font-weight: 600; line-height: 16px; vertical-align: baseline;
    }}
    .rail-empty {{ font-size: 12px; color: var(--text-secondary); padding: 4px 11px; }}
    .council-rail .rail-filter {{
    ...
    .council-rail .rail-filter:focus {{ outline: none; border-color: var(--action); box-shadow: 0 0 0 3px rgba(79, 144, 149, 0.12); }}
    /* Desktop (>=1024px): the councils nav is a persistent left rail; main
       reserves its width so the content re-centers in the remaining space
       (claude.ai pattern). The hamburger COLLAPSES it (body.rail-collapsed) and
       main reclaims full width; no scrim on desktop. */
    @media (min-width: 1024px) {{
      .council-rail {{ transform: translateX(0); }}
      body > main {{ margin-left: var(--rail-w, 288px); }}
      body.rail-collapsed .council-rail {{ transform: translateX(-100%); }}
      body.rail-collapsed > main {{ margin-left: 0; }}
      .rail-scrim {{ display: none !important; }}
    }}
    /* Mobile / narrow (<=1023px, incl. the Chrome side panel): the rail is an
       OFF-CANVAS DRAWER over a scrim — opened by the hamburger (body.rail-open),
       floating ABOVE main (no reflow), never stacked in-page. Replaces the old
       in-page-rail (<=1179) + below-main-rail (<=560) blocks. */
    @media (max-width: 1023px) {{
      body > main {{ margin-left: 0; }}
      .council-rail {{ width: min(var(--rail-w, 288px), 86vw); box-shadow: 0 0 40px rgba(0, 0, 0, 0.18); }}
      body.rail-open .council-rail {{ transform: translateX(0); }}
    }}
    /* Cross-provider eval leaderboard rows. The desktop grid packs six
       columns (rank · provider · n · BAR · score · judge); on a phone the fixed
       columns eat the whole row and the 1fr score-bar — the central "which model
       wins YOUR corpus" wedge — collapses to 0px while the judge column clips off
       the right edge. Keep the bar visible by tightening the fixed columns and
       dropping the secondary judge column (it's already disclosed in the footer
       "Judges are rotated…" line) below 560px.
       The desktop text columns are CONTENT-SIZED (auto), not the old fixed
       80px/70px tuned for raw slugs: post-#275 the judge cell renders
       "judge: <BRAND>" ("judge: Gemini" measures ~94px), so a fixed 70px wrapped
       it onto a 2nd line and every leaderboard row ballooned to ~47px tall — a
       ragged, double-height block on the exact "proven on your rejections" card a
       journalist screenshots. auto columns fit the brand on ONE line; the bar
       keeps a minmax(80px,1fr) floor so it stays the dominant wedge without
       starving the labels. Founder symptom: "judge column wrapped, tall ragged
       rows on /stats". */
    .eval-lb-row {{ grid-template-columns: 24px minmax(56px, auto) auto minmax(80px, 1fr) auto auto; }}
    /* The compact "(self)" flag on the NAME cell is the mobile counterpart of the
       wide ".eval-lb-judge" column: hidden on desktop (the judge column already
       shows "(self)" there — no double-tag), shown below 560px once the judge
       column drops, so the self-judge disclosure stays IDENTIFIABLE at phone
       widths instead of the footer referencing a "(self)" tag no row displays. */
    .eval-lb-self-compact {{ display: none; }}
    @media (max-width: 560px) {{
      .eval-lb-row {{ grid-template-columns: 18px minmax(56px, auto) 38px minmax(40px, 1fr) 54px; gap: 6px; }}
      .eval-lb-judge {{ display: none; }}
      .eval-lb-self-compact {{ display: inline; }}
    }}
    /* Browser-capture per-provider rows — same fixed-grid-eats-the-1fr-bar shape
       as the eval leaderboard above. The desktop grid is provider · count · BAR ·
       "N unsynced"; the fixed 140+60+110 + 3×8px gaps = 334px, so at <=393px the
       1fr distribution BAR collapses to 0px AND the right-edge "unsynced" sync-diff
       pill clips past the viewport (a page-level h-overflow on a phone — the
       founder mobile-grid-clip shape). Tighten the fixed columns + give the bar a
       minmax floor below 480px so the bar stays visible and the row never exceeds
       the viewport; the pill keeps its title tooltip. */
    .bc-provider-row {{ grid-template-columns: 140px 60px 1fr 110px; }}
    @media (max-width: 480px) {{
      .bc-provider-row {{ grid-template-columns: minmax(64px, 1.2fr) 34px minmax(36px, 1fr) auto; gap: 6px; }}
    }}
    /* Eval per-axis breakdown rows (REFRAME/REDIRECT/SHARPENING/COMPRESSION) —
       the SAME fixed-grid-eats-the-1fr-bar shape as the two rows above. The
       desktop grid is axis-name(110) · n(50) · BAR(1fr) · score(80); the fixed
       110+50+80 + 3×8px gaps = 264px, so at 320px the 1fr axis-score BAR (the
       "which axis the model is strongest on" visual) collapses to 0px and at
       375px it's a barely-readable 43px. Tighten the fixed columns + give the bar
       a minmax floor below 480px so the bar stays a real width on a phone. */
    .eval-axis-row {{ display: grid; grid-template-columns: 110px 50px 1fr 80px; gap: 8px; align-items: center; font-size: 13px; }}
    @media (max-width: 480px) {{
      .eval-axis-row {{ grid-template-columns: minmax(72px, auto) 34px minmax(40px, 1fr) auto; gap: 6px; }}
    }}
  </style>

  <aside class="council-rail" id="council-rail">
    <!-- The heading + search box are STICKY (.rail-header) so a long council
         history (up to 500 rows) can be scrolled without the filter scrolling
         out of reach — founder symptom: "scrolled the councils list and the
         search box was just gone, had to scroll all the way back to filter".
         An opaque background + the negative-margin bleed keep scrolling rows
         from showing through the pinned header. -->
    <div class="rail-header">
      <h2>Councils</h2>
      <input type="search" id="rail-filter" class="rail-filter"
             placeholder="Search councils…" aria-label="Search councils by title"
             autocomplete="off">
    </div>
    {recent_sidebar}
    <p class="rail-empty" id="rail-no-match" style="display: none;">No councils match that search.</p>
    <!-- A11y status mirror for the rail filter (WCAG 4.1.3 Status Messages, AA).
         The visual #rail-no-match line above is toggled via display, and toggling
         an already-present static string from display:none→'' is NOT reliably
         announced (the text never CHANGED, only its visibility) — so a keyboard /
         screen-reader user who typed a query that matched zero councils heard
         SILENCE while the list emptied (a sighted user reads the line; a blind user
         gets no feedback the filter found nothing, AND no feedback when matches
         return). This PERSISTENT region exists from first render and only its TEXT
         mutates (the reliable announce pattern — same shape as #liveAnnouncement and
         the memory viewer's #sr-status), so the rail filter IIFE speaks the result
         count. Lives OUTSIDE the petite-vue app (the rail is a sibling of <main>),
         so it can't reuse the Vue-bound liveAnnouncement — it gets its own region. -->
    <div id="rail-filter-status" class="sr-only" role="status" aria-live="polite" aria-atomic="true"></div>
  </aside>

  <main>
    <div class="launchpad-shell lp-view-{view}" id="launchpad-app" v-scope="LaunchpadApp(pageData)" v-cloak @vue:mounted="init">
      <!-- Councils side-nav toggle (hamburger) + the mobile drawer scrim. The
           <aside class="council-rail"> is a sibling of <main>; this fixed button
           toggles it through the runtime — body.rail-collapsed on desktop,
           body.rail-open + this scrim on mobile. Inside the v-scope so petite-vue
           owns the click; both are position:fixed so DOM order is irrelevant. -->
      <!-- aria-expanded/-controls so a screen reader announces whether the
           council-history rail is currently SHOWN or HIDDEN (WCAG 4.1.2 Name/
           Role/Value). Without it AT heard a bare "Toggle councils, button" with
           no state — the asymmetric sibling of the disclosure-state gaps already
           closed for the memory-viewer thread rep (memory_viewer.py aria-expanded
           lockstep) and the live-council round divider (council_review.py:1475).
           The state is DUAL-MODE: on desktop the rail shows by default and the
           toggle COLLAPSES it (expanded = !railCollapsed); on narrow it's a drawer
           HIDDEN by default and the toggle OPENS it (expanded = railOpen) — so
           railExpanded folds the breakpoint in (see the getter). -->
      <button class="rail-toggle" type="button" @click="toggleRail()"
              :aria-expanded="railExpanded ? 'true' : 'false'" aria-controls="council-rail"
              aria-label="Toggle councils" title="Councils (history)">☰</button>
      <div class="rail-scrim" v-if="railOpen" @click="closeRail()"></div>
      <!-- A11y status mirror (WCAG 4.1.3 Status Messages, AA). The launchpad's
           dynamic-status surfaces — the dispatch-failure banner (dispatchBannerOpen)
           and the launch-status / launchError card — appear/update IN RESPONSE TO A
           USER ACTION (clicking Launch) but carry no live-region role, so a
           screen-reader user clicked Launch, the dispatch failed, and they were told
           NOTHING (a sighted user sees the banner; a blind user hears silence). This
           PERSISTENT region exists from first render (a live region inserted
           already-populated is not reliably announced; this one is present at load
           and only its TEXT mutates), so every dispatch-failure / launch transition
           is spoken. Mirrors the live-council fix (council_review.py, sweep iter 98)
           onto the launchpad's own dynamic status. .sr-only is the shared
           visually-hidden-but-AT-available utility in SHARED_CSS. -->
      <div id="live-announcement" class="sr-only" role="status" aria-live="polite" aria-atomic="true">{{{{ liveAnnouncement }}}}</div>
      <!-- Stats-page header — only visible on /stats (stats-card). Gives the
           analytics page its own title + a way back to the simple council home. -->
      <section class="card stats-card" style="border-left: 3px solid #7fa0ad; background: rgba(43, 80, 112, 0.04);">
        <div class="eyebrow">{DIE_MARK_INLINE_SVG} Trinity · stats</div>
        <h1 style="margin-top: 4px;">Trinity · stats</h1>
        <p class="lede">Routing, cortex picks, evals, memory health, capture, and import — the full diagnostics surface.</p>
        <p class="meta" style="margin-top: 8px;"><a href="./launchpad.html">← Back to the council</a></p>
      </section>

      <section class="card hero-shell home-card">
        <div>
          <div class="eyebrow">{DIE_MARK_INLINE_SVG} Trinity</div>
          <h1>{{{{ heroTitle }}}}</h1>
          <p class="lede">{{{{ heroLede }}}}</p>
          <p class="meta hero-mechanism" v-if="heroMechanism">{{{{ heroMechanism }}}}</p>
        </div>
        <button type="button" @click="settingsOpen ? closeSettings() : openSettings()" style="background: none; border: none; cursor: pointer; padding: 8px; opacity: 0.7; flex-shrink: 0; min-width: 44px; min-height: 44px; display: inline-flex; align-items: center; justify-content: center;" title="Settings" aria-label="Open settings">
          <span aria-hidden="true" style="font-size: 24px; line-height: 1;">⚙</span>
        </button>
      </section>

      <!-- Cold-start aha + council value proof. Extracted from the hero so the
           narrow side panel can put the council DISPATCH directly under the hero
           and demote this evidence below it (action-first). Self-hides on a cold
           install where both signals are null. -->
      <section class="card home-card hero-proof" v-if="pageData.coldOpen || pageData.councilValue">
          ...
          <p class="cold-open" v-if="pageData.coldOpen"
             style="margin-top: 10px; padding: 10px 14px; border-left: 3px solid var(--accent, #4f9095); background: rgba(79,144,149,0.06); font-size: 14px; line-height: 1.5;">
            🪞 {{{{ pageData.coldOpen }}}}
          </p>
          <!-- DEGRADED-honesty: the cold-open names a lens TENSION (or the cached
               taste-signature adjectives) — both EMBEDDING-DERIVED. Under the SHA-1
               TF-IDF fallback (a fresh install with no [mlx] extras — NORMAL
               operation), the lens still surfaces a tension, but it's a "caricature
               of your taste, not it" (lens_health._embedding_backend → DEGRADED). The
               cold-open is the FIRST and most prominent lens claim on the home view —
               it paints ~590px ABOVE the taste card's own trust banner, so a user
               reads "the tension you keep navigating" as settled truth long before
               any honesty. Carry the SAME lensTrust-gated banner here so the loudest
               embedding-derived claim can't green-while-degraded (#35). Gated on
               coldOpen so it never fires next to the council-derived councilValue
               (which is NOT embedding-derived). ONE source of truth with the CLI
               `lens-health` verb (pageData.lensTrust reuses the same probe). -->
          <div class="lp-trust-banner"
               v-if="pageData.coldOpen && pageData.lensTrust && pageData.lensTrust.embedder_degraded">
            <span class="lp-trust-label">degraded</span>
            <span class="lp-trust-hint">{{{{ pageData.lensTrust.summary || 'Built on the SHA-1 TF-IDF fallback embedder — lexical (keyword) vectors, not semantic ones, so this tension is a caricature of your taste, not it.' }}}}<span v-if="pageData.lensTrust.fix"> Fix: <code>{{{{ pageData.lensTrust.fix }}}}</code></span></span>
          </div>
          ...
          <p class="council-value" v-if="pageData.councilValue"
            ...
            <!-- The denominator here is `comparable` (real head-to-head contests
                 where BOTH a chairman winner AND your default were recorded), NOT
                 your total council count. /stats below labels its DIFFERENT base
                 (`councils_aggregated`, every label-bearing council) "councils"
                 too, and `status` calls this same `comparable` subset "real
                 contests" — so the bare word "councils" here made one ledger read
                 as two different totals (12 here vs 17 on /stats). Name the grain
                 ("head-to-head councils") so the subset can't be mistaken for the
                 user's council total or collide with the /stats count. Same
                 grain-naming fix as the #278 wedge "by area:". -->
            ⚖ Across your <b>{{{{ pageData.councilValue.councils }}}}</b> head-to-head councils
            <span class="meta" title="Real contests where both the chairman's winner and your default were recorded — the councils where a single-tab pick can actually be compared. Your full council history (including walkovers and unlabeled runs) is larger; see /stats.">(ⓘ)</span>,
            the synthesized winner differed from your default <b>{{{{ pageData.councilValue.changedPct }}}}%</b> of the time —
            ...
            <!-- the asymmetric wedge: which lab wins each broad AREA. This is a
                 COARSE family rollup (head token of the chairman task_type —
                 code_gen + code_review → "code"), a strictly broader grain than
                 the /stats "Best by task type" cheat-sheet. #278: labeling it "by
                 kind" collided with the cheat-sheet's "per kind of question"
                 framing, so a reader mapped the wedge's "code→Claude" onto the
                 cheat-sheet's "code_gen→GPT" and read the two as a contradiction
                 — the SAME word, a DIFFERENT (finer) grain, a legitimately
                 different leader. Name the grain ("by area") + a tooltip that says
                 it rolls up several task types so the two surfaces don't read as
                 disagreeing. Self-hides when no family clears the volume+margin floor. -->
            <span class="meta" style="display: block; margin-top: 2px;"
                  v-if="pageData.councilValue.wedge && pageData.councilValue.wedge.length"
                  title="Broad areas — each rolls up several task types (code_gen + code_review → code). For the per-task-type winner, see the cheat-sheet under /stats; a finer task can have a different leader than its area.">
              by area:
      ...
      <!-- Home → /stats link (home-card). The simple home page leads with the
           council; everything analytical lives one click away on /stats. -->
      <!-- No opacity dimming: --text-muted/--action are already AA-calibrated
           de-emphasis tokens; stacking opacity on top double-dims muted text
           (and this nav link) below WCAG AA 4.5 (the stats link fell to 2.8:1).
           UX sweep 2026-06-21 — the opacity-on-muted-text contrast class. -->
      <p class="meta home-card" style="text-align: center; margin-top: -12px; font-size: 13px;">
        <a href="./stats.html">View full stats &amp; analytics →</a>
      </p>

      <section class="settings-modal" v-if="settingsOpen" @click.self="closeSettings()" role="dialog" aria-modal="true" aria-label="Settings" style="position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.5); display: flex; align-items: flex-start; justify-content: center; overflow-y: auto; padding: 24px 12px; z-index: 1000;">
        <div class="card" style="max-width: 460px; margin: 20px; position: relative;">
          <!-- The × close button is the modal's PRIMARY dismiss affordance on a
               touch surface (the side panel + phones, where backdrop-click is
               far less discoverable). It shipped as a bare 26×30px glyph (no
               min-size, no ::before hit-area) — below the 44px touch floor every
               OTHER action button clears (the settings gear that OPENS this modal
               is 44×44; .icon-action/.button/.copy-badge all carry the floor). It
               was the one icon-action the 44px-hit-area sweep missed because it's
               a bare inline-styled button, not a .icon-action. Flex-center a 44×44
               box so the × stays visually in the corner while the tap target meets
               the founder's "every action button clears 44px on touch" directive. -->
          <button @click="closeSettings()" style="position: absolute; top: 6px; right: 6px; min-width: 44px; min-height: 44px; display: inline-flex; align-items: center; justify-content: center; background: none; border: none; cursor: pointer; font-size: 24px; line-height: 1; opacity: 0.6; transition: opacity 0.2s; z-index: 10;" aria-label="Close settings">×</button>
          <div class="eyebrow">Settings</div>
          <h2>Launchpad controls</h2>
          <p class="meta">Telemetry is on by default — toggle it off anytime. Trinity can share anonymous Launchpad views and routing labels — the task type, the winning provider, the council's member count and mode, plus aggregate provider win-rates — never raw prompts, outputs, code, or file paths.</p>
          <!-- The active-benefit nudge promises a CROSS-USER outcome ("surface
               broken flows across users"). That only happens when sharing is BOTH
               consented AND wired to a collector — i.e. shareIsLive. A stock public
               install ships no GA4 creds, so the endpoint is "Not configured" and
               maybeSendTelemetry() never fires: the nudge would overclaim a benefit
               that can't happen. Gate it on shareIsLive, and tell the not-live user
               the honest truth instead (consent on, nothing leaving this build). -->
          <p v-if="shareIsLive" class="meta" style="margin-top: 8px; padding: 10px 12px; background: rgba(79, 144, 149, 0.06); border-left: 3px solid rgba(79, 144, 149, 0.4); border-radius: 0 6px 6px 0; font-size: 13px;">
            <b>Catch more issues like the bugs you've hit by keeping sharing on.</b>
          ...
          <!-- v-else-if (not bare v-else): shareIsLive is false in TWO states —
               (a) enabled but no endpoint, and (b) sharing toggled OFF. This line
               says "Sharing is on", so it must only show in state (a); a bare
               v-else would falsely claim "Sharing is on" the moment the user turns
               the toggle OFF (the same self-contradiction this nudge fix targets).
               When disabled, neither nudge shows — the toggle + descriptive copy
               above already convey the off state. -->
          <p v-else-if="telemetry.enabled" class="meta" style="margin-top: 8px; padding: 10px 12px; background: rgba(79, 144, 149, 0.06); border-left: 3px solid rgba(79, 144, 149, 0.4); border-radius: 0 6px 6px 0; font-size: 13px;">
            <b>Nothing is leaving this machine.</b>
            Sharing is on, but this build has no collector endpoint configured (see Endpoint below), so no anonymous events are sent. Your consent is remembered for if that changes.
          </p>
          ...
          <div class="settings-list">
            ...
            <!-- Reset's no-extension CLI-copy confirmation renders HERE, inline
                 beside the button it confirms — not 155px down at the sharing
                 toggle with toggle-worded copy (which read as the wrong action). -->
            <p class="meta" v-if="copiedKey === 'settings-reset-cli'" style="margin: -4px 0 0; color: var(--accent-deep);">✓ Copied — run it to assign a new anonymous ID (the current ID stays until you do)</p>
            <div class="setting-row">
          ...
          <div class="sharing-toggle">
            <!-- State-aware label, NOT a static "Sharing enabled" literal. The body
                 status paragraph above (v-else-if="telemetry.enabled") was already
                 careful never to claim "Sharing is on" once the toggle is OFF; the
                 toggle's OWN label was the asymmetric sibling that still read
                 "Sharing enabled" beside an UNCHECKED switch when sharing was
                 disabled — a self-contradiction the user (and, via aria-labelledby,
                 the screen reader naming this checkbox) sees. Bind it to the same
                 telemetry.enabled signal the checkbox keys on so name + state agree. -->
            <span class="meta" id="sharing-toggle-label">{{{{ telemetry.enabled ? 'Sharing enabled' : 'Sharing disabled' }}}}</span>
            <label class="toggle-switch">
              <input type="checkbox" aria-labelledby="sharing-toggle-label" :checked="telemetry.enabled" @change="toggleSharing">
              <span class="toggle-slider"></span>
            </label>
          </div>
          <!-- Without the Chrome extension the toggle can't apply the setting
               itself, so triggerSettingsAction copies the CLI command instead.
               This line confirms the copy AND tells the user the toggle is
               unchanged until they run it — so the displayed state stays honest. -->
          <p class="meta" v-if="copiedKey === 'settings-cli'">✓ Copied — run it in your terminal to apply (the toggle stays as-is until you do)</p>
      ...
      <section
        ...
        style="border-left: 3px solid #4f9095; background: rgba(45, 106, 79, 0.05);"
      >
        <div class="eyebrow" style="color: #34666b;">
      ...
      <section class="card home-card" v-if="pageData.lensBuild"
               style="border-left: 3px solid #4f9095; background: rgba(79,144,149,0.04);">
        <h2 style="margin-top: 4px; font-size: 18px;">
          <span v-if="pageData.lensBuild.building">🧩 Building your lens…</span>
          <span v-else-if="pageData.lensBuild.status === 'complete' && pageData.lensBuild.lensPopulated">✓ Your lens is ready</span>
          <span v-else-if="pageData.lensBuild.status === 'complete'">Lens build finished — no tensions yet</span>
          <span v-else-if="pageData.lensBuild.status === 'canceled'">⏸ Lens build stopped</span>
          <span v-else>⚠ Lens build hit a snag</span>
        </h2>
        <p class="meta" style="margin-top: 6px;"
           v-if="(pageData.lensBuild.status !== 'failed' && pageData.lensBuild.status !== 'complete') || pageData.lensBuild.error">
          <!-- The stage label is the LAST pipeline stage's name. On terminal
               states it can CONTRADICT the honest header, so suppress it there:
                 - 'failed' → bare key "failed" (no STAGES mapping), reads as
                   "failed — …" under "⚠ Lens build hit a snag".
                 - 'complete' → the build ALWAYS ends on stage "done", whose label
                   is the SUCCESS string "Lens ready" (lens_progress.STAGES). But a
                   complete build that found NO tensions renders the honest header
                   "Lens build finished — no tensions yet", so a "Lens ready" label
                   line directly below it re-asserts the success the header denied
                   (green-while-degenerate, #35 — the exact contradiction
                   _lens_build_for_launchpad's docstring forbids). The header
                   already states the complete outcome authoritatively (ready /
                   no-tensions yet), so the stage label adds nothing on EITHER
                   complete path — drop it for both. The running/canceled states
                   keep their descriptive stage label (it shows WHERE a cancel
                   stopped). The (user-safe, type-only) error clause is independent
                   of the label so it still shows on any state that carries one. -->
          <span v-if="pageData.lensBuild.status !== 'failed' && pageData.lensBuild.status !== 'complete'">{{{{ pageData.lensBuild.label }}}}</span><span v-if="pageData.lensBuild.error"><span v-if="pageData.lensBuild.status !== 'failed' && pageData.lensBuild.status !== 'complete'"> — </span>{{{{ pageData.lensBuild.error }}}}</span>
        </p>
        <p class="meta" v-if="pageData.lensBuild.status === 'complete' && !pageData.lensBuild.lensPopulated" style="margin-top: 4px;">
          Trinity needs more of your decisions to find stable taste tensions — keep using your tools and the lens will fill in.
        </p>
        <div v-if="pageData.lensBuild.building"
             style="margin-top: 8px; height: 8px; background: rgba(79,144,149,0.12); border-radius: 4px; overflow: hidden;">
          <div :style="{{ width: pageData.lensBuild.pct + '%', height: '100%', background: '#4f9095', transition: 'width 0.4s' }}"></div>
        </div>
        <div style="margin-top: 10px; display: flex; gap: 8px; align-items: center;">
          <button type="button" class="button ghost" v-if="pageData.lensBuild.building"
            ...
            <span v-if="lensBuildAction === 'stopping'">Stopping…</span><span v-else-if="lensBuildAction === 'stop-failed'">Retry stop</span><span v-else>Stop</span>
          </button>
          <button type="button" class="button ghost"
            ...
            <span v-if="lensBuildAction === 'restarting'">Restarting…</span><span v-else-if="lensBuildAction === 'restart-failed'">Retry restart</span><span v-else>Restart</span>
          </button>
          <span class="meta" v-if="pageData.lensBuild.building && lensBuildAction !== 'stop-failed'">Reload to refresh progress · safe to close — it runs in the background.</span>
        </div>
        <p class="meta" v-if="lensBuildAction === 'stop-failed' || lensBuildAction === 'restart-failed'"
           style="margin-top: 8px; color: #b3261e;">
          ⚠ Couldn't {{{{ lensBuildAction === 'stop-failed' ? 'stop' : 'restart' }}}} the lens build<span v-if="lensBuildError">: {{{{ lensBuildError }}}}</span>. Try again, or run <code>trinity-local {{{{ lensBuildAction === 'stop-failed' ? 'lens-stop' : 'lens --force' }}}}</code> in your terminal.
        </p>
      </section>
      ...
      <!-- Build-deeper-memory card — surfaces ONLY when the user has
           ...
           View-class is MODE-CONDITIONAL (UX sweep iter 81 — the #273
           "don't let degradation be silent" trigger was re-buried by the
           home/stats split):
             - mode 'download'      = the model was NEVER fetched → a pure opt-in
               UPSELL ("get sharper basins"). `stats-card` → /stats only; the
               simple home stays a clean council surface and the lens cards carry
               a one-line "Sharper meaning needs the embedder — see stats" pointer.
             - mode 'reinstall-libs' = the weights ARE cached but the libs broke /
               a venv switched → Trinity "silently fell back to lexical keyword
               matching" (the exact #273 silent-degradation this card exists to
               surface). That is a REGRESSION, not an upsell, so it must show on
               HOME, not be hidden behind a /stats click whose only home trace is
               the same static pointer a fully-working install shows. Untagged →
               visible on BOTH views. -->
      <section
        :class="'card embedder-status-card ' + (pageData.embedderStatus && pageData.embedderStatus.mode === 'reinstall-libs' ? 'embedder-degraded-card' : 'stats-card')"
        v-if="pageData.embedderStatus && pageData.embedderStatus.show"
        :style="pageData.embedderStatus && pageData.embedderStatus.mode === 'reinstall-libs' ? 'border-left: 3px solid #b3261e; background: rgba(179, 38, 30, 0.04);' : 'border-left: 3px solid #4f9095; background: rgba(79, 144, 149, 0.04);'"
      >
        <!-- Two states, two honest cures (UX sweep iter 44):
             - mode 'download'      → the model is MISSING, a real ~600 MB fetch.
             - mode 'reinstall-libs' → the weights are ALREADY cached but the
               MLX libs (torch/sentence-transformers) broke / a venv switched
               (the #273 trigger). Re-running `huggingface-cli download` would
               re-fetch 600 MB the user already has — the cure is just the pip
               install. Show that, not a fake "download". -->
        <!-- Eyebrow tells the truth per mode: 'download' is an OPTIONAL upsell;
             'reinstall-libs' is a SILENT degradation (#273) the user should treat
             as a problem, not an optional extra — don't label a regression "Optional". -->
        <div class="eyebrow" v-if="pageData.embedderStatus.mode !== 'reinstall-libs'" style="color: #34666b;">Optional · deeper memory</div>
        <div class="eyebrow" v-else style="color: #b3261e;">Embeddings degraded · using lexical fallback</div>
        <h2 v-if="pageData.embedderStatus.mode !== 'reinstall-libs'" style="margin-top: 4px; font-size: 18px;">
          Build deeper memory (~600 MB download)
        </h2>
        <h2 v-else style="margin-top: 4px; font-size: 18px;">
          Re-enable deeper memory (no re-download)
        </h2>
        <p v-if="pageData.embedderStatus.mode !== 'reinstall-libs'" class="meta" style="margin-top: 8px;">
          The model (modernbert-embed-base, ~600 MB) reads <em>meaning</em>,
          not just words — it sharpens your lens basins and turns on the
          semantic flows (taste lens, correction lens, the noise filter).
          Without it, lens-build still runs but falls back to lexical
          keyword matching: those semantic flows stay off and the basins
          are coarser. The council launchpad works fully either way.
        </p>
        <p v-else class="meta" style="margin-top: 8px;">
          The model is <em>already downloaded</em> — but the embedding libs
          (sentence-transformers / torch) aren't importable (a broken install
          or a venv switch), so Trinity silently fell back to lexical keyword
          matching: the semantic flows (taste lens, correction lens, the noise
          filter) are off and your basins are coarser. Reinstall the libs to
          turn them back on — <strong>no 600 MB re-download</strong>, the
          weights are cached. The council launchpad works fully either way.
        </p>
        <div class="provider-command" style="margin-top: 10px;">
          ...
          <button
            ...
            :title="pageData.embedderStatus.mode === 'reinstall-libs' ? 'Copy reinstall command' : 'Copy download command'"
            :aria-label="pageData.embedderStatus.mode === 'reinstall-libs' ? 'Copy reinstall command' : 'Copy download command'"
          >
            <span v-if="copiedKey === 'embedder-download'">✓</span>
            <span v-else>⧉</span>
          </button>
        </div>
        <p v-if="pageData.embedderStatus.mode !== 'reinstall-libs'" class="meta" style="margin-top: 8px; font-size: 12px;">
          One-time download. Reload this page after the command finishes.
        </p>
        <p v-else class="meta" style="margin-top: 8px; font-size: 12px;">
          Reinstalls the libs only — the cached weights stay. Reload this page after the command finishes.
        </p>
      </section>
      ...
      <section
        ...
        style="border-left: 3px solid #7fa0ad; background: rgba(43, 80, 112, 0.04);"
      >
        <!-- #7fa0ad (--info) is the FILL token (the 3px border-left, the eval bars)
             — at 2.2:1 it's unreadable as eyebrow TEXT. Use --accent-deep #34666b
             (5.1:1 on the tinted card), the same readable token the other eyebrows
             carry. The info-blue-as-text / fill-token-as-text class. -->
        <div class="eyebrow" style="color: var(--accent-deep, #34666b);">
          <span v-if="inExtensionPanel">Finish setup · optional</span>
          <span v-else>Cross-bootstrap · optional</span>
        </div>
        <!-- In the side panel the user is ALREADY inside the installed extension,
             so "Install the Chrome extension" reads as a contradiction (founder-
             class: a card telling you to do what you've plainly already done). The
             real remaining step there is registering the Native-Messaging host via
             `trinity-local install-extension`. Reframe headline + lead-in on the
             inExtensionPanel signal; the file:// launchpad keeps the install pitch. -->
        <h2 style="margin-top: 4px; font-size: 18px;" v-if="inExtensionPanel">
          Wire up browser capture to finish the extension setup
        </h2>
        <h2 style="margin-top: 4px; font-size: 18px;" v-else>
          Install the Chrome extension for browser capture + auto-update
        </h2>
        <p class="meta" style="margin-top: 8px;" v-if="inExtensionPanel">
          The extension is installed (you're in its side panel) — one step left to
          turn on capture: run <code>trinity-local install-extension --extension-id &lt;ID&gt;</code>
          (the popup shows your ID) to register the Native-Messaging host. Then
          conversations from claude.ai / chatgpt.com / gemini.google.com flow into
          <code>~/.trinity/conversations/</code> automatically.
        </p>
        ...
        <!-- Web Store published → one-click "Add to Chrome". Until then
             ...
             CHROME_WEB_STORE_URL constant flips this. SKIPPED in the side panel
             (inExtensionPanel) — "Add to Chrome"/sideload steps are install-the-
             extension guidance, and the extension is already installed there; the
             reframed body above names the one remaining step (install-extension). -->
        <p class="meta" style="margin-top: 8px;" v-if="!inExtensionPanel && pageData.browserExtension.webStoreUrl">
        ...
        <p class="meta" style="margin-top: 8px;" v-else-if="!inExtensionPanel">
          See <a href="https://github.com/keepwhatworks/trinity/blob/main/docs/INSTALL-extension.md"
            target="_blank" style="color: var(--accent-deep, #34666b);">docs/INSTALL-extension.md</a>
      ...
      <section class="launch-grid home-card">
        <article class="card">
          <div class="eyebrow">Council</div>
          <h2>Ask every model at once</h2>
          <p class="meta">Every model you use — frontier and local — answers in parallel. A local chairman synthesizes — agreed claims, disagreed claims with <em>why each matters</em>, picked winner. Every council sharpens the local router automatically — no clicks, no ratings; you just chat.</p>
          <label class="label mb-sm" for="council-prompt">Task</label>
          <!-- ⌘/Ctrl+Enter submits — the same keyboard convention the council
               REFINE composer (council_review.py) already ships + advertises.
               The primary composer is the focal element a first-time eye lands
               on (CSS flex order), yet a keyboard user had NO way to launch from
               it: they typed a prompt and had to mouse to (or Tab past every
               card to) "Launch Council" — the secondary composer one click
               deeper was MORE keyboard-operable than the headline one. Bare
               Enter stays a newline (council prompts are multi-line); .prevent
               stops the modified Enter from inserting one. -->
          <textarea
            id="council-prompt"
            v-model="prompt"
            @input="onPromptInput"
            @keydown.enter.meta.prevent="launchCouncil"
            @keydown.enter.ctrl.prevent="launchCouncil"
            placeholder="Ask a council question… (⌘/Ctrl+Enter to launch)"
          ></textarea>

          <p class="meta" v-if="polishHintVisible" style="background: rgba(43, 80, 112, 0.08); border-left: 3px solid #7fa0ad; padding: 8px 12px; margin-top: 12px; border-radius: 4px;">
            💡 Polish task detected. Click "Auto-chain" on the council page to iterate up to 3 rounds toward convergence.
          </p>

          <div class="actions" style="margin-top: 18px;">
            <!-- When a council is already running the button is disabled — but a
                 bare greyed button gave NO feedback (founder: "clicked Launch
                 Council while another ran, it did nothing"). Relabel + tooltip so
                 the disabled state explains itself; Open/Stop live in the panel below. -->
            <button type="button" class="button primary" @click="launchCouncil" :disabled="busy"
                    :title="launchButtonTitle">{{{{ launchButtonLabel }}}}</button>
          </div>
          <p class="meta" style="margin-top: 12px; font-size: 13px;">
            Or drive Trinity from inside Claude Code: type <code>/trinity</code> after running <code>trinity-local install-mcp</code>. Same chairman, no tab switch.
              ...
              <section class="launch-status" role="status" aria-live="polite" v-if="operation || launchError">
                ...
                <div class="launch-status-actions" v-if="operation">
                  ...
                  <button type="button" class="button ghost" v-if="operation?.kind === 'council' && busy" :disabled="stopRequested" @click="stopCurrentCouncil">{{{{ stopRequested ? 'Stopping…' : 'Stop council' }}}}</button>
        ...
        <table class="routing-table cortex-rules-table cortex-cheat-sheet" style="margin-top: 16px;">
          <tbody>
            ...
            <tr v-for="r in cortexRules.rules"
                :style="r.margin < cortexRules.winner_margin_floor ? 'opacity: 0.55;' : null"
                :title="r.margin < cortexRules.winner_margin_floor ? 'Thin margin (' + r.margin_str + ' < ' + cortexRules.winner_margin_floor + ' gate): the runner-up was close, so ask falls back to kNN for this basin until more councils sharpen it.' : 'ask routes this basin (margin ' + r.margin_str + ' ≥ ' + cortexRules.winner_margin_floor + ' gate).'">
      ...
      <!-- This is the CHART card — it only earns its space once there's routing
           ...
           councils" paragraph that DUPLICATED the dedicated empty-state card right
           below (`v-if="!personalRoutingTable"`, "Run a few councils to learn which
           model works best for you") — same eyebrow, same `trinity-local council
           --task` command, stacked back-to-back (REDUNDANT, found driving cold
           /stats). Gate the whole chart card on the data existing so the dedicated
           empty-state card OWNS the cold state (it has the richer copy-command CTA),
           and the two never co-render. -->
      <section class="card stats-card" v-if="personalRoutingTable && personalRoutingTable.councils_aggregated">
        <div class="eyebrow">Routing</div>
        <h2>Which model wins for which kind of question</h2>
        <p class="meta">
          From your own {{{{ personalRoutingTable.councils_aggregated }}}} council{{{{ personalRoutingTable.councils_aggregated === 1 ? '' : 's' }}}} — the bars sharpen with every council. Categories match LMArena so you can compare against public evals later.
        </p>
        ...
        <details style="margin-top: 16px;" v-if="personalRoutingTable && personalRoutingTable.councils_aggregated">
          <summary class="meta" style="cursor: pointer;">Local Elo · current models only (older generations excluded)</summary>
          <!-- Thin-data caveat: a 2-game-each chart is a coin-flip, not a ranking.
               The bars render as deviation from the 1500 base (centered axis) so a
               near-base flip is a small bar at the midline, not a tower floored at
               1400 — but the caveat still has to say so. DEMOTE-not-HIDE, matching
               the eval-hero "Thin sample" + cheat-sheet "n=2" siblings (Trinity
               council council_21a5b74fb1df3fda, unanimous). -->
          <!-- Single-entrant degeneracy: an Elo chart is a RELATIVE ranking, so
               one lonely bar floating above the 1500 base is not a ranking at
               all — there's nothing to rate it against, and the opponents it
               beat are hidden below the games floor. Suppress the meaningless
               one-bar chart + per-bar caption and explain instead (mirrors the
               routing cheat-sheet's "No routing pattern yet" ghost-table
               suppression + the eval per-axis-leader contest gate). The data
               layer also stops EMITTING a ranking here so any other consumer
               inherits the gate. -->
          <p class="meta" v-if="eloChart && eloChart.degenerate" style="margin-top: 8px;">
            Only <strong>{{{{ eloChart.soloLabel }}}}</strong> has enough councils ({{{{ eloChart.minGames }}}}, the floor is {{{{ eloChart.gamesFloor }}}}) to plot an Elo rating — the models it ran against are still below the floor, so there's no head-to-head ranking to show yet. Run a few more councils with at least two models each and the chart fills in.
          </p>
          <!-- The non-degenerate ranking. Each child carries the `!degenerate`
               guard inline (NOT wrapped in a single <template v-if> — petite-vue
               mishandles a multi-child <template v-if> and throws an insertBefore
               null error that blanks the whole page). -->
          <p class="meta" v-if="eloChart && !eloChart.degenerate && eloChart.thin"
             style="margin-top: 8px; background: rgba(79, 144, 149, 0.08); border-left: 3px solid #b26a1f; padding: 8px 12px; border-radius: 4px;">
            ⚠️ Thin sample — your weakest-played model has only {{{{ eloChart.minGames }}}} council{{{{ eloChart.minGames === 1 ? '' : 's' }}}} (under {{{{ eloChart.thinFloor }}}}). Treat this as provisional, not a ranking — a single coin-flip moves the bars. The gaps sharpen as you run more councils.
          </p>
          <p class="meta" v-if="eloChart && !eloChart.degenerate" style="margin-top: 8px;">Bars show each model's rating relative to the 1500 starting point.</p>
          <div class="chart-shell" v-if="eloChart && !eloChart.degenerate" style="margin-top: 8px;">
            <canvas id="provider-elo-chart"></canvas>
          </div>
          <!-- Per-bar game-count disclosure: a 2-game bar must not read as
               authoritative as a 250-game one. The chart itself carries no n, so
               surface it as a caption (the cheat-sheet table already shows n=N). -->
          <p class="meta" v-if="eloChart && !eloChart.degenerate && eloChart.games && eloChart.games.length" style="margin-top: 8px; font-size: 0.85em;">
            <span v-for="(label, i) in eloChart.labels" :key="label" style="margin-right: 14px;">
              <strong>{{{{ label }}}}</strong> · {{{{ eloChart.elos[i] }}}} Elo · n={{{{ eloChart.games[i] }}}}
            </span>
          </p>
        </details>
      </section>
      ...
      <section class="card stats-card" v-if="personalRoutingTable">
        ...
        <p class="meta">
          The same "which model for what" answer, broken out by task type — from your own {{{{ personalRoutingTable.councils_aggregated || 0 }}}} council{{{{ (personalRoutingTable.councils_aggregated || 0) === 1 ? '' : 's' }}}}. The chairman synthesizes through your taste lens and picks the answer YOU would have picked; the <strong>Best</strong> column is who it picks most often (and how often). <strong>Personalization</strong> is how much this pick leans on YOUR councils vs the global default — it climbs from near 0% toward 100% as you run more councils on that task type. The per-provider scores trail behind as supporting detail.
        </p>
        <p class="meta" v-if="coreStatus.state === 'stale'" style="background: rgba(79, 144, 149, 0.08); border-left: 3px solid #b26a1f; padding: 8px 12px; margin-top: 12px; border-radius: 4px;">
          ⚠️ Your <code>core.md</code> is stale — a memory file was updated since the last distillation. Run <code>trinity-local lens --only-distill</code> (~20s) so the chairman reads the freshest synthesis on the next council.
        </p>
        <p class="meta" v-if="coreStatus.state === 'missing'" style="background: rgba(43, 80, 112, 0.08); border-left: 3px solid #7fa0ad; padding: 8px 12px; margin-top: 12px; border-radius: 4px;">
          💡 You have core memories but no distillation yet. Run <code>trinity-local lens --only-distill</code> to produce <code>~/.trinity/core.md</code> — the one paragraph chairmen read first.
        </p>
        <!-- Routing data EXISTS but EVERY task_type is a single council (n=1) — so
             routingCheatSheetMap (n>=2 filtered) is empty. Without this gate the
             table renders a 5-column HEADER row with ZERO body rows — a header-only
             ghost table that reads as broken/loading, with the real "all single-
             council" explanation buried in the hidden-count note below. Suppress the
             ghost table and show an honest empty-state (mirrors the memory-viewer
             routing reader's "No routing pattern yet" sibling). -->
        <p class="meta" v-if="!routingCheatSheetHasRows" style="margin-top: 16px;">
          No routing pattern yet — all {{{{ routingHiddenSingletons }}}} task type{{{{ routingHiddenSingletons === 1 ? '' : 's' }}}} so far {{{{ routingHiddenSingletons === 1 ? 'has' : 'have' }}}} just a single council each (one council isn't a track record). Run a few more councils on the same kind of question and the pattern sharpens. Open <a :href="'../portal_pages/memory.html?file=routing.json'" style="color: inherit; border-bottom: 1px dotted var(--text-muted);">routing.json</a> for every entry.
        </p>
        <table class="routing-table" v-if="routingCheatSheetHasRows">
          <thead>
            <tr>
              <th>Task type</th>
              <th>Best</th>
              <th style="text-align: right;"
                  title="How much this pick leans on YOUR councils vs the global default. Climbs from near 0% toward 100% as you run more councils on this task type.">Personalization</th>
              <th v-for="provider in routingTableProviders" style="text-align: right;">{{{{ formatProviderLabel(provider) }}}}</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="(scores, taskType) in routingCheatSheetMap">
              <td>
              ...
              <td>
                <!-- Confident pick: a strict chairman-win plurality. The chip
                     reads "the chairman picks X here". -->
                <span class="suggestion-chip" v-if="personalRoutingTable.best_per_task_type[taskType] && !pickIsTie(taskType)">{{{{ formatProviderLabel(personalRoutingTable.best_per_task_type[taskType]) }}}}</span>
                <!-- Tie / coin-flip (2-2, 1-1-1, or no chairman supervision):
                     demote to a muted "Lean X · no clear pick" so the column
                     doesn't paint a 33%/50% split as a confident "Best"
                     (green-gate #35; mirrors the picks "Lean · near-tie"). -->
                <span class="benchmark-unit" v-if="personalRoutingTable.best_per_task_type[taskType] && pickIsTie(taskType)"
                      title="The chairman split evenly here — no clear pick yet. Needs more councils to break the tie.">
                  Lean {{{{ formatProviderLabel(personalRoutingTable.best_per_task_type[taskType]) }}}} · no clear pick
                </span>
                <span class="benchmark-unit" v-if="!personalRoutingTable.best_per_task_type[taskType]" title="No clear chairman pick yet — needs more councils for this task type to call a best">—</span>
                <!-- Chairman wins per task_type: "picked 3 of 5"
                     ...
                     falls back to the chip alone. For a tie the count
                     reads "tied 2 of 4" so the split is legible. -->
                <span class="meta" v-if="winsFor(taskType)" style="display: block; font-size: 11px; font-weight: 400; margin-top: 2px;">
                  {{{{ pickIsTie(taskType) ? 'tied' : 'picked' }}}} {{{{ winsFor(taskType).wins }}}} of {{{{ winsFor(taskType).total }}}}
                </span>
              </td>
              <td style="text-align: right;"
                  :title="coldStartFor(taskType) ? ('This pick leans ' + coldStartFor(taskType).personalization_pct + '% on YOUR ' + coldStartFor(taskType).n_personal + ' council' + (coldStartFor(taskType).n_personal === 1 ? '' : 's') + ' for this task type, the rest on the global default. Run more councils here to raise it.') : null">
        ...
        <!-- "+ N hidden" note: only when there ARE visible rows above it (else the
             all-single-council empty-state above already explains the suppression —
             a "+N hidden" line under an empty-state would read as if rows existed). -->
        <p class="meta" v-if="routingHiddenSingletons > 0 && routingCheatSheetHasRows" style="margin-top: 10px;">
          + {{{{ routingHiddenSingletons }}}} task type{{{{ routingHiddenSingletons === 1 ? '' : 's' }}}} with only a single council hidden — one council isn't a routing pattern yet. Open <a :href="'../portal_pages/memory.html?file=routing.json'" style="color: inherit; border-bottom: 1px dotted var(--text-muted);">routing.json</a> for the full table.
        </p>
      </section>
      ...
      <section class="card stats-card" v-if="!personalRoutingTable">
        ...
        <div class="code-copy-wrap">
          ...
          <button type="button" class="copy-badge" title="Copy command" aria-label="Copy command"
                  @click="copyCodeBlock($event, 'copy-council-cta')">
            <span v-if="copiedKey === 'copy-council-cta'">✓ Copied</span>
      ...
      <section class="card new-models-card stats-card"
               v-if="pageData.newModels && pageData.newModels.length"
               style="margin-bottom: 18px; border-left: 3px solid rgba(79, 144, 149, 0.55); background: rgba(79, 144, 149, 0.05);">
        <div class="eyebrow" style="color: #34666b;">New model 🎉</div>
        <h2 style="margin-top: 4px; font-size: 18px;">
        ...
        <div v-for="nm in pageData.newModels" :key="nm.slug" style="margin-bottom: 10px;">
          ...
          <!-- Re-score vs first-score: a model the user ALREADY benchmarked (on a
               now-superseded version) gets re-score copy that LEADS with the answer
               — "your benchmark is for the old model" — so the card doesn't read as
               "you have no number yet" while the leaderboard one screen below shows
               the (stale) score under the old model name. (UX sweep iter 14.)
               The old model is NAMED by the card header (nm.display) + the leaderboard
               row; we deliberately do NOT print the recorded `target_model` API id
               here — it's a raw lowercase-hyphenated slug (`claude-opus-4-6`,
               `gpt-5.2-codex`) the user can't act on, and it reads as a leaked code
               symbol in a celebratory nudge whose whole pitch is human-readable.
               (UX sweep iter 199 — opaque-id-as-label class, sibling of #184/#187.) -->
          <p v-if="nm.rescore" class="meta" style="margin: 0 0 6px; font-size: 12px;">
            Your benchmark is for the previous version — re-run to refresh your number.
          </p>
      ...
      <section class="card eval-empty-state-card stats-card"
               v-if="pageData.evalSummary && !pageData.evalSummary.has_results"
               style="margin-bottom: 18px; border-left: 3px solid rgba(79, 144, 149, 0.4); background: rgba(79, 144, 149, 0.03);">
        <!-- workstream #116 — internal ref kept here, NOT in the rendered
             eyebrow: a new user has no idea what a parenthesised task number
             means and it reads as unfinished. -->
        <div class="eyebrow" style="color: #34666b;">Cross-provider benchmark</div>
        <h2 style="margin-top: 4px; font-size: 18px;">
          Score the 3 providers on YOUR corpus
        </h2>
        <p class="meta" style="margin-top: 4px; margin-bottom: 12px;">
          Global benchmarks (HumanEval, MMLU) tell you which model is strongest in the
          aggregate. Trinity scores Claude / GPT / Gemini against
          <strong>your own rejection patterns</strong> — the same evidence the chairman uses
          to pick winners in your councils.
        </p>
        <!-- State A — no rejections in the ledger yet. Lead with `lens`: it MINES
             the rejections eval-build needs from transcripts the user already has.
             eval-build would FileNotFoundError here, so it must NOT lead (the
             2026-06-07 persona-audit first-run dead-end). -->
        <div v-if="!pageData.evalSummary.rejections_available" style="margin-bottom: 10px;">
          <p class="meta" style="margin-bottom: 8px; font-size: 13px;">
            Trinity scores providers on the times you <strong>rewrote</strong> their answer.
            First, build your lens — it mines those rejections from the transcripts you already have:
          </p>
          <button type="button"
                  class="suggestion-chip"
                  style="font-family: ui-monospace, monospace; font-size: 13px; cursor: pointer; padding: 6px 12px;"
                  @click.stop="copyText('trinity-local lens', 'eval-lens-empty')">
            <span v-if="copiedKey === 'eval-lens-empty'">✓ Copied</span>
            <span v-else>trinity-local lens</span>
          </button>
          <p class="meta" style="margin-top: 12px; margin-bottom: 6px; font-size: 12px;">
            <strong>No local transcripts?</strong> Ask each provider for their view directly —
            quality &gt; quantity, 10–25 is plenty:
          </p>
        ...
        <!-- State B — rejections exist (lens ran / imports landed) but no eval set
             built yet. Now eval-build works. -->
        <div v-else-if="!pageData.evalSummary.eval_set_available" style="margin-bottom: 10px;">
          <p class="meta" style="margin-bottom: 8px; font-size: 13px;">
            Build the eval set from your saved rejections:
          </p>
          <button type="button"
                  class="suggestion-chip"
                  style="font-family: ui-monospace, monospace; font-size: 13px; cursor: pointer; padding: 6px 12px;"
      ...
      <!-- The eval moat — PROMOTED to a lead value card (was demoted behind a
           <details> pre-2026-06-07). This is the one artifact no model vendor and
           no request-router can reconstruct: it scores providers on the user's
           OWN rejection signal (the times they REWROTE an answer — a correction
           that happens in the editor, never as an API call a gateway could see).
           Audience-neutral by design: the SAME proof reads as "which model to
           ask" for a consumer on flat plans and "which model to route per-token"
           for an enterprise — one app, one benefit, no $ language here. -->
      <section class="card eval-summary-card stats-card" v-if="pageData.evalSummary && pageData.evalSummary.has_results"
               style="margin-bottom: 18px; border-left: 3px solid #4f9095; background: rgba(79, 144, 149, 0.04);">
        <div class="eyebrow" style="color: #34666b;">Your benchmark · proven on your rejections</div>
        <p class="meta" style="margin: 4px 0 6px; font-size: 13px;">
          The one signal no model vendor or request-router can copy — which model wins
          the questions <strong>you rewrote</strong>, scored on your own corpus, locally.
        </p>
        <h2 style="margin-top: 4px; font-size: 18px;">
          {{{{ pageData.evalSummary.target_display || pageData.evalSummary.target }}}}
          ...
          <span style="float: right; font-variant-numeric: tabular-nums;">
            <span v-if="pageData.evalSummary.aggregate_score !== null">
              {{{{ pageData.evalSummary.aggregate_score_str }}}}
            </span>
        ...
        <p class="meta" style="margin-top: 4px;">
          ...
          {{{{ pageData.evalSummary.items_total }}}} item{{{{ pageData.evalSummary.items_total === 1 ? '' : 's' }}}} from your rejection signal
          <span v-if="pageData.evalSummary.total_runs > 1"> · {{{{ pageData.evalSummary.total_runs }}}} runs on disk</span>
        </p>
        <!-- Honesty: when the leaderboard rows span DIFFERENT eval sets, the hero
             number is just the top score among non-comparable runs — NOT a
             head-to-head "best model" win. The mixed-set warning below is gated on
             the comparison block (>=2 rows); the HERO is the number a journalist
             screenshots, so the caveat must ride the hero too (confidence-honesty:
             demote a mixed-set claim on EVERY claim surface, not just the table). -->
        <p class="meta" v-if="pageData.evalSummary.mixed_eval_sets"
           style="margin-top: 4px; color: var(--warning-text, #79591b); background: rgba(138,109,59,0.08); border-left: 3px solid #c8a24a; padding: 6px 9px; border-radius: 4px; font-size: 12px;">
          ⚠ Top score across <strong>different</strong> eval sets — not a head-to-head win.
          See the leaderboard below; re-run each provider on one set for a comparable claim.
        </p>
        <!-- Hero low-confidence demotion (green-gate #35, council_4416b36956074e21):
             the headline number IS the screenshot a journalist takes, so a thin
             run (a 1-9 item smoke) must self-demote rather than present a clean
             "0.83 · the one signal no vendor can copy" claim. Two INDEPENDENT
             failure modes, each its own caveat (the council was explicit that
             low-n and axis-dominance are separate). The score still renders above;
             these only add the honesty band. Same amber treatment as the per-axis
             low-n dimming — confidence-honesty: demote on EVERY claim surface. -->
        <p class="meta" v-if="pageData.evalSummary.hero_low_confidence"
           style="margin-top: 4px; color: var(--warning-text, #79591b); background: rgba(138,109,59,0.08); border-left: 3px solid #c8a24a; padding: 6px 9px; border-radius: 4px; font-size: 12px;">
          ⚠ Thin sample — only <strong>{{{{ pageData.evalSummary.hero_items_completed }}}}</strong>
          item<span v-if="pageData.evalSummary.hero_items_completed !== 1">s</span> scored
          (under {{{{ pageData.evalSummary.hero_min_items }}}}). Treat as a smoke run, not a
          publishable benchmark — re-run with more items via
          <code>trinity-local eval-run --target {{{{ pageData.evalSummary.target }}}}</code>.
        </p>
        <p class="meta" v-if="pageData.evalSummary.hero_dominant_axis"
           style="margin-top: 4px; color: var(--warning-text, #79591b); background: rgba(138,109,59,0.08); border-left: 3px solid #c8a24a; padding: 6px 9px; border-radius: 4px; font-size: 12px;">
          ⚠ One axis dominates — <strong>{{{{ pageData.evalSummary.hero_dominant_axis }}}}</strong>
          is {{{{ Math.round(pageData.evalSummary.hero_dominant_axis_share * 100) }}}}% of the
          scored items, so this is mostly a {{{{ pageData.evalSummary.hero_dominant_axis }}}}
          score, not a balanced "YOUR kind of question" score. Build a more even eval set
          with <code>trinity-local eval-build</code>.
        </p>
        <!-- #303: the headline shows your STRONGEST scored model (the value
             proof). When the most-recent run was a different (often weaker,
             just-benchmarked) model, surface it here so the "you just scored a
             new model" freshness isn't lost — without letting a 0.50 latest run
             bury a 0.79 winner in the hero. -->
        <p class="meta"
           v-if="pageData.evalSummary.latest_run && pageData.evalSummary.latest_run.aggregate_score !== null && pageData.evalSummary.latest_run.aggregate_score !== undefined"
           style="margin-top: 2px;">
          Most recent run: {{{{ pageData.evalSummary.latest_run.target_display }}}} scored {{{{ pageData.evalSummary.latest_run.aggregate_score_str }}}}
          <span>· headline shows your strongest</span>
        </p>
        <ul style="list-style: none; padding: 0; margin: 12px 0 0; display: flex; flex-direction: column; gap: 4px; font-variant-numeric: tabular-nums;">
          ...
          <li v-for="axis in pageData.evalSummary.axes" :key="axis.name"
              class="eval-axis-row"
              :style="axis.count < 3 ? 'opacity: 0.4;' : null"
            ...
            <span style="position: relative; height: 6px; background: rgba(0,0,0,0.06); border-radius: 3px;">
              <span :style="'position: absolute; left: 0; top: 0; bottom: 0; width: ' + (axis.mean * 100) + '%; background: #7fa0ad; border-radius: 3px;'"></span>
            </span>
            <span style="text-align: right;">{{{{ axis.mean_str }}}}</span>
          </li>
        </ul>
        ...
        <div v-if="pageData.evalSummary.comparison && pageData.evalSummary.comparison.length >= 2"
          ...
          <div v-if="pageData.evalSummary.mixed_eval_sets"
               style="margin: 8px 0; padding: 8px 10px; background: rgba(79, 144, 149, 0.08); border-left: 3px solid #4f9095; border-radius: 4px; font-size: 12px;">
          ...
          <div v-if="pageData.evalSummary.per_axis_leader && pageData.evalSummary.per_axis_leader.length >= 1"
            ...
            <span v-for="chip in pageData.evalSummary.per_axis_leader" :key="chip.axis"
                  style="display: inline-block; padding: 3px 8px; background: rgba(79, 144, 149, 0.08); border-radius: 4px; font-size: 11px; font-weight: 600; color: #34666b; font-variant-numeric: tabular-nums;">
              {{{{ chip.axis }}}}: {{{{ chip.target_display || chip.target }}}} {{{{ chip.score_str }}}}
            </span>
          </div>
          <ul style="list-style: none; padding: 0; margin: 8px 0 0; display: flex; flex-direction: column; gap: 4px; font-variant-numeric: tabular-nums;">
            <li v-for="(row, i) in pageData.evalSummary.comparison" :key="row.target"
                class="eval-lb-row"
                style="display: grid; gap: 8px; align-items: center; font-size: 13px;">
              <span class="meta" style="text-align: right;">{{{{ i + 1 }}}}.</span>
              <span style="font-weight: 600;">{{{{ row.target_display || row.target }}}}<!--
                   Compact self-judge flag on the NAME cell — the ONE column that
                   survives the <560px reflow (the wide ".eval-lb-judge" column,
                   which carried the only "(self)" marker, is display:none on a
                   phone). Without this, the footer note "a row tagged (self) was
                   graded by its own model family" dangled on mobile: it named a
                   tag NO ROW showed, so a phone reader couldn't tell the WINNING
                   row was self-graded — the iter-446/448 self-judge disclosure
                   defeated by the mobile reflow. ".eval-lb-self-compact" is the
                   mirror of ".eval-lb-judge": shown ONLY <560px (so desktop keeps
                   its single "(self)" in the judge column, no double-tag). -->
                <span class="eval-lb-self-compact" v-if="row.self_judge" style="color: var(--warning-text, #79591b); font-weight: 600;" title="This run's judge is the same provider family as the target — the model graded its own family. Measured non-self-preferential, so the score still ranks; surfaced for transparency."> (self)</span></span>
              <span class="meta">n={{{{ row.items_completed }}}}</span>
              <span style="position: relative; height: 6px; background: rgba(0,0,0,0.06); border-radius: 3px;">
                <span v-if="row.aggregate_score !== null"
                      :style="'position: absolute; left: 0; top: 0; bottom: 0; width: ' + (row.aggregate_score * 100) + '%; background: #4f9095; border-radius: 3px;'"></span>
              </span>
              <span style="text-align: right;">
                <span v-if="row.aggregate_score !== null">{{{{ row.aggregate_score_str }}}}</span>
                <span v-else class="meta">—</span>
              </span>
              <!-- Self-judge marker: when a row's judge slug == its target slug
                   (e.g. `eval-run --target claude --judge claude`), the "judge:
                   Claude" label sits next to a "Claude" row name two columns left —
                   the model graded its own family. Without the inline "(self)" tag
                   the row read as a clean cross-judged result while the footer
                   claimed "a model never grades itself", contradicting itself on a
                   public card. Measured non-self-preferential, so this is neutral
                   transparency (amber, not red). -->
              <span class="meta eval-lb-judge" style="text-align: right;" v-if="row.judge">judge: {{{{ row.judge_display || row.judge }}}}<span v-if="row.self_judge" style="color: var(--warning-text, #79591b);" title="This run's judge is the same provider family as the target — the model graded its own family. Measured non-self-preferential, so the score still ranks; surfaced for transparency."> (self)</span></span>
              <span class="meta eval-lb-judge" style="text-align: right;" v-else></span>
            </li>
          </ul>
          <!-- Excluded-items disclosure: the aggregate is a mean over
               COMPLETED items only (timeouts / dispatch failures are dropped,
               not scored 0), and providers can fail DIFFERENT items — so two
               aggregates can span slightly different subsets of the eval set.
               Matches the CLI `eval-show --compare` note and the single-
               provider detail view's "N/M dispatched, K failed". Hides itself
               when every provider scored its whole set. The ranking surface
               must not read as more apples-to-apples than the data is. -->
          <div v-if="pageData.evalSummary.excluded_runs && pageData.evalSummary.excluded_runs.length >= 1"
               class="meta" style="margin-top: 8px; font-size: 12px; color: #8a6d2f;">
            ⚠ Excluded from the aggregate (timed out / dispatch error, not scored):
            <span v-for="(ex, i) in pageData.evalSummary.excluded_runs" :key="ex.target_display"><span v-if="i > 0">, </span>{{{{ ex.target_display }}}}: {{{{ ex.items_failed }}}}</span>.
            Means are over completed items only; providers may fail different items.
          </div>
          <!-- Footer judge-policy note. The "a model never grades itself" claim
               is only true when NO displayed row is self-judged — a user can force
               `eval-run --target claude --judge claude`, persisting self_judge=True,
               and the row then renders "judge: Claude (self)". Asserting the
               absolute claim with such a row present contradicts the row itself on
               a public, screenshottable card. Gate the claim on !any_self_judge and
               disclose the self-judge case instead (mirrors the CLI eval-run note:
               "self-judge — same family as target, measured non-self-preferential").
               -->
          <p class="meta" style="margin-top: 8px; font-size: 12px;" v-if="!pageData.evalSummary.any_self_judge">
          ...
          <p class="meta" style="margin-top: 8px; font-size: 12px;" v-else>
            Each row is the most-recent <code>eval-run</code> per target. Judges
            default to a different provider, but a row tagged <strong>(self)</strong>
            was graded by its own model family (measured non-self-preferential, so it
            still ranks). The strongest score on YOUR rejections wins, not the global
            leaderboard.
          </p>
        </div>
        <p class="meta" style="margin-top: 12px;">
          <code>trinity-local eval-show --compare</code> renders the same leaderboard;
          <code>--compare --by-axis</code> adds the per-axis matrix (Claude wins REFRAME, GPT wins COMPRESSION, etc.);
      ...
      <section class="card taste-card home-card" v-if="tasteLenses">
        <div class="eyebrow" style="display: flex; align-items: center; gap: 8px;">
        ...
        <p class="meta">
          ...
          the principles you redirect away from. Refreshes when lens runs.
        </p>
        <!-- Consistency pointer: the embedder card (the ~600 MB model that
             sharpens semantic basins) moved to /stats, so name the dependency
             here so it isn't invisible from the simple home page. One line, not
             a card. -->
        <p class="meta" style="margin-top: 4px; font-size: 12px;">
          Sharper meaning needs the embedder — see <a href="./stats.html">stats</a>.
        </p>

        <!-- Embedder-degraded trust banner (#35 green-while-degraded). When
             lens_health._embedding_backend() is DEGRADED (the SHA-1 TF-IDF
             fallback — a fresh install WITHOUT the [mlx] extras, NORMAL
             operation), the lens built on lexical (keyword) vectors, so its
             tensions are "caricatures of your taste, not it" (lens_health). The
             confident "The patterns in how you think" header above would
             otherwise paint that caricature as settled truth — exactly the green
             the memory viewer's .viewer-trust-banner refuses. ONE source of
             truth with the CLI `lens-health` verb (pageData.lensTrust reuses the
             same _embedding_backend probe) so the two surfaces can't disagree.
             Scoped to the embedding-derived taste card only (NOT the council
             painkiller / raw council counts / eval scores, which aren't
             embedding-derived). -->
        <div class="lp-trust-banner"
             v-if="pageData.lensTrust && pageData.lensTrust.embedder_degraded">
          <span class="lp-trust-label">degraded</span>
          <span class="lp-trust-hint">{{{{ pageData.lensTrust.summary || 'Built on the SHA-1 TF-IDF fallback embedder — lexical (keyword) vectors, not semantic ones, so these tensions are caricatures of your taste, not it.' }}}}<span v-if="pageData.lensTrust.fix"> Fix: <code>{{{{ pageData.lensTrust.fix }}}}</code></span></span>
        </div>
          ...
          <ol class="taste-list">
            <li v-for="(p, idx) in tasteLenses.paired_lenses" :key="'pair-' + idx">
              ...
              <span v-if="p.verdict"
                    ...
                    :style="'display: inline-block; margin-left: 8px; padding: 1px 6px; font-size: 10px; letter-spacing: 0.04em; text-transform: uppercase; border-radius: 3px; ' + (p.verdict === 'accepted' ? 'background: rgba(79, 144, 149, 0.08); color: #34666b;' : 'background: rgba(43, 80, 112, 0.08); color: #34666b;')"
                    :title="p.verdict === 'accepted' ? 'Built from your local rejection corpus via trinity-local lens' : 'Imported from a provider via trinity-local lens-import'">
                {{{{ p.verdict === 'accepted' ? 'lens' : ('via ' + ((p.dual_evidence && p.dual_evidence.source_provider && p.dual_evidence.source_provider[0] && formatProviderLabel(p.dual_evidence.source_provider[0])) || 'provider')) }}}}
              </span>
              ...
              <span v-if="p.support_count"
                    class="meta"
                    :style="'display: inline-block; margin-left: 8px; padding: 1px 6px; font-size: 10px; letter-spacing: 0.04em; text-transform: uppercase; border-radius: 3px; ' + (p.low_confidence ? 'background: rgba(176, 122, 30, 0.10); color: #8a6d1e;' : 'background: rgba(43, 80, 112, 0.08); color: #34666b;')"
              ...
              <!-- Spans-basins chips: each basin id deep-links to the
                   ...
                   The chip BODY names the basin (its top terms, e.g.
                   "design · arch"), NOT the opaque internal id "b00" — the
                   cheat-sheet row already resolved this (cheatSheetLabel,
                   _topology_basin_labels docstring "b03 alone is opaque"),
                   and on TOUCH (the side panel + phones) there's no hover,
                   so a raw-id chip body left the only explanation (the
                   :title) permanently unreachable. The id rides the :title
                   for traceability. Plain .cross-memory-chip (NOT --id /
                   monospace) since the body is now prose, not a short id. -->
              <span class="lens-basins-row" v-if="p.basins_spanned && p.basins_spanned.length"
                ...
                <span class="meta" style="font-size: 10px; letter-spacing: 0.04em; text-transform: uppercase;">Spans</span>
                <a v-for="bid in p.basins_spanned" :key="'lb-' + idx + '-' + bid"
                   class="lens-basin-chip cross-memory-chip"
              ...
              <span class="lens-decisions-row"
                ...
                <span class="meta" style="font-size: 10px; letter-spacing: 0.04em; text-transform: uppercase;">Justified by · click to see the work</span>
                  ...
                  <details v-for="did in p.tension_decisions"
                    ...
                    <div
                      ...
                      <blockquote
                        ...
                        style="margin: 4px 0 0; padding: 4px 0 4px 10px; border-left: 2px solid rgba(120, 99, 70, 0.2); color: #5e666f; font-style: italic;">
                    ...
                    <div
                      style="margin-top: 6px; padding: 6px 8px; font-size: 11px; color: #5e666f; font-style: italic;"
                      v-else>
                      Decision {{{{ did }}}} not found in the preference-act ledger — lens may be stale; re-run trinity-local lens.
                    </div>
                  </details>
                  <!-- "+N more" per-lens decision-chip note: the
                       tension_decisions list is capped server-side
                       (_TASTE_DECISIONS_PER_LENS_CAP) so a deeply-supported
                       tension can't render dozens of inline <details>
                       expanders (the dominant page-wall component). The full
                       justification set rides the lens.md viewer. -->
                  <span class="meta" v-if="p.tension_decisions_hidden"
                        style="font-size: 11px; align-self: center;">
                    +{{{{ p.tension_decisions_hidden }}}} more
                  </span>
                </span>
              </span>
            </li>
          </ol>
          <!-- "+N more" hidden-count note (unbounded-render class, Iter 251):
               the paired-lens list is capped server-side (_TASTE_PAIRED_CAP) so
               a deep power-user lens can't wall the minimal home into a 16k+px
               scroll. Surface what was hidden + an escape to the FULL lens
               (memory.html?file=lens.md renders every tension, uncapped) so the
               cap is honest, not a silent truncation. Mirrors the #290
               cheat-sheet "+N hidden" + Raw-JSON-escape pattern. -->
          <p class="meta taste-more-note" v-if="tasteLenses.paired_lenses_hidden"
             style="margin: 8px 0 0; font-size: 12px;">
            +{{{{ tasteLenses.paired_lenses_hidden }}}} more {{{{ tasteLenses.paired_lenses_hidden === 1 ? 'tension' : 'tensions' }}}} —
            <a href="../portal_pages/memory.html?file=lens.md">see your full lens</a>.
          </p>
        </div>
        ...
        <div class="taste-block" v-if="tasteLenses.orderings && tasteLenses.orderings.length">
          ...
          <ul class="taste-list">
            <li v-for="(o, idx) in tasteLenses.orderings" :key="'ord-' + idx">
              ...
              <span v-if="o.verdict"
                    :style="'display: inline-block; margin-left: 8px; padding: 1px 6px; font-size: 10px; letter-spacing: 0.04em; text-transform: uppercase; border-radius: 3px; ' + (o.verdict === 'preserve_as_ordering' ? 'background: rgba(79, 144, 149, 0.08); color: #34666b;' : 'background: rgba(43, 80, 112, 0.08); color: #34666b;')"
                    :title="o.verdict === 'preserve_as_ordering' ? 'Built from your local rejection corpus via trinity-local lens (preserve_as_ordering — preference without dual evidence)' : 'Imported from a provider via trinity-local lens-import'">
                {{{{ o.verdict === 'preserve_as_ordering' ? 'lens' : ('via ' + ((o.dual_evidence && o.dual_evidence.source_provider && o.dual_evidence.source_provider[0] && formatProviderLabel(o.dual_evidence.source_provider[0])) || 'provider')) }}}}
              </span>
            </li>
          </ul>
          <!-- "+N more" note for orderings — same cap+escape pattern as the
               paired-lens note above (unbounded-render class, Iter 251). -->
          <p class="meta taste-more-note" v-if="tasteLenses.orderings_hidden"
             style="margin: 8px 0 0; font-size: 12px;">
            +{{{{ tasteLenses.orderings_hidden }}}} more {{{{ tasteLenses.orderings_hidden === 1 ? 'ordering' : 'orderings' }}}} —
            <a href="../portal_pages/memory.html?file=lens.md">see your full lens</a>.
          </p>
        </div>
        ...
        <div class="taste-block" v-if="tasteLenses.rejections && tasteLenses.rejections.length">
          ...
          <p class="meta taste-more-note" v-if="tasteLenses.rejections_hidden"
             style="margin: 8px 0 0; font-size: 12px;">
            +{{{{ tasteLenses.rejections_hidden }}}} more —
            <a href="../portal_pages/memory.html?file=lens.md">see your full lens</a>.
          </p>
        </div>
        ...
        <div class="taste-block" v-if="tasteLenses.abstract_lenses.length">
          ...
          <p class="meta taste-more-note" v-if="tasteLenses.abstract_lenses_hidden"
             style="margin: 8px 0 0; font-size: 12px;">
            +{{{{ tasteLenses.abstract_lenses_hidden }}}} more —
            <a href="../portal_pages/memory.html?file=lens.md">see your full lens</a>.
          </p>
        </div>
        ...
        <div class="taste-vocab" v-if="tasteLenses.vocabulary.length">
          ...
          <span class="meta" v-if="tasteLenses.vocabulary_hidden" style="font-size: 12px; margin-left: 4px;">+{{{{ tasteLenses.vocabulary_hidden }}}} more</span>
        </div>
        ...
        <div class="taste-share-row">
          ...
          <button class="button ghost taste-share-btn" @click="renderMeCard" :disabled="busy || meCardStatus === 'rendering'" style="margin-left: 8px;">
            <span v-if="meCardStatus === 'rendering'">Rendering…</span>
            <span v-else-if="meCardStatus === 'done'">✓ Rendered — opening</span>
          ...
          <!-- Co-located PNG-render FAILURE feedback. renderMeCard used to flash
               "✓ Rendered — opening" OPTIMISTICALLY (before the dispatch resolved),
               so a FAILED render (PIL missing / host down) still claimed success on
               the button while the real error landed in the unrelated council-launch
               ribbon up top. Now the button shows a pending "Rendering…" cue and the
               failure surfaces HERE, next to the button that triggered it. -->
          <p class="status-error taste-mecard-error" v-if="meCardStatus === 'error'" style="margin-top: 8px;">
            {{{{ meCardError || 'Could not render the PNG card.' }}}} The card lives at ~/.trinity/share/me_card.png — run <code>trinity-local me-card</code> to retry.
          </p>
        </div>
      </section>

      <section class="card home-card" v-if="!tasteLenses">
        ...
        <!-- Consistency pointer (empty-state mirror): the embedder card moved
             to /stats, so name the semantic dependency here too. One line. -->
        <p class="meta" style="margin-top: 4px; font-size: 12px;">
          Sharper meaning needs the embedder — see <a href="./stats.html">stats</a>.
        </p>
        <div class="code-copy-wrap">
          ...
          <button type="button" class="copy-badge" title="Copy command" aria-label="Copy command"
                  @click="copyCodeBlock($event, 'copy-lens-cta')">
            <span v-if="copiedKey === 'copy-lens-cta'">✓ Copied</span>
            <span v-else>Copy</span>
          </button>
        </div>
      ...
      <!-- Council history MOVED to the persistent left rail (.council-rail) —
           founder 2026-06-01: "why dup council history on the main page." The
           sidebar is now the single home for the full council list (no limit);
           the old in-page card grid + its title-filter + show-more + the
           discarded card-build plumbing were fully removed 2026-06-06 (they
           rendered nothing — the grid element never existed in this page). -->
      ...
      <section class="card stats-card" v-if="pageData.timeline && pageData.timeline.length">
        <h2 style="margin-top: 4px; font-size: 18px;">🗓 Your timeline</h2>
        <p class="meta" style="margin-top: 6px;">
          The arcs your prompts trace through time — your most-substantial topic
          surges, oldest first.
        </p>
        <div style="margin-top: 10px; display: flex; flex-direction: column; gap: 6px;">
          <!-- flex-wrap:wrap + a range min-width that floors at 0 below 150px of
               room (the narrow column) — UX sweep iter 92. At 320px the card's
               inner box is ~284px and three non-shrinking columns (130px range +
               12 gap + label + 12 gap + ~89 "N prompts") overran it: the prompts
               .meta span painted to x≈375, 55px PAST the 320 viewport (a real
               horizontal-overflow defect the iter-45 note misattributed as
               "unrelated to the timeline card"). flex-wrap lets the prompts count
               drop to a second line in-card instead of spilling off-screen; the
               clamp keeps the 130px range column on wide widths but stops it
               hard-pinning the row wider than the card on a phone. -->
          <div v-for="ch in pageData.timeline" :key="ch.start"
               style="display: flex; flex-wrap: wrap; align-items: baseline; gap: 4px 12px; font-size: 14px;">
            <span class="mono" style="color: var(--text-muted, #5e666f); min-width: clamp(0px, 130px, 100%); flex-shrink: 1;">{{{{ ch.range }}}}</span>
            <span style="flex: 1 1 auto; min-width: 0;"><b>{{{{ ch.label }}}}</b></span>
      ...
      <section class="card memory-health-card" v-if="memoryHealth && memoryHealth.issues && memoryHealth.issues.length">
        <div class="eyebrow" style="display: flex; align-items: center; gap: 8px;">
          ...
          <!-- "Refresh memory" button (council_1f9cbecd7104f90f priority #3).
               ...
               COST-BLIND class (UX sweep iter 54, the Auto-chain sibling):
               dream is ~10+ flagship model calls over several minutes on the
               user's OWN subscription quota — the button must say so, not look
               like a free one-click refresh. Tooltip + the co-located cost
               line below carry the honest cost signal. -->
          <button type="button"
                  class="suggestion-chip"
          ...
          <!-- #147 self-healing UI surface: "Repair extension" button.
               ...
               COST-BLIND class (UX sweep iter 54): extension-repair-auto
               dispatches a FULL council (every installed provider + chairman)
               on the user's subscription — disclose it, don't frame it as a
               free one-click fix. -->
          <button type="button"
                  class="suggestion-chip"
                  style="padding: 4px 10px; font-size: 12px; cursor: pointer;"
                  :disabled="repairExtensionStatus === 'running'"
                  title="Dispatches a full council (all your providers) on your own subscription"
        ...
        <!-- Co-located cost signal (UX sweep iter 54 — the Auto-chain
             cost-blind sibling). Refresh memory dispatches `dream` (~10+
             flagship model calls over several minutes) and Repair extension
             dispatches a full council (every provider) — both ride YOUR own
             subscription, the product's whole cost basis. The buttons read
        ...
        <!-- Co-located failure detail (#242(a) class): the WHY a Refresh /
             ...
             read "⚠ Failed" and the real reason leaked to the council ribbon. -->
        <p class="meta" v-if="(refreshMemoryStatus === 'failed' && refreshMemoryError) || (repairExtensionStatus === 'failed' && repairExtensionError)"
           style="margin-top: 6px; color: var(--warning-text, #79591b); font-size: 12px;">
          ⚠ {{{{ refreshMemoryStatus === 'failed' ? 'Refresh memory' : 'Repair extension' }}}} couldn't dispatch: {{{{ refreshMemoryStatus === 'failed' ? refreshMemoryError : repairExtensionError }}}}. Try again, or run it from your terminal.
        </p>
        <h2 style="margin-top: 4px; font-size: 18px;">
        ...
        <ul class="memory-health-list" style="list-style: none; padding: 0; margin: 12px 0 0; display: flex; flex-direction: column; gap: 6px;">
          <li v-for="issue in memoryHealth.issues" :key="issue.name + issue.status"
              style="display: flex; align-items: baseline; gap: 10px; padding: 8px 12px; background: rgba(79, 144, 149, 0.06); border-left: 3px solid var(--warning, #bd9658); border-radius: 0 6px 6px 0; font-size: 13px; flex-wrap: wrap;">
            <code style="font-size: 12px; color: var(--accent-deep, #34666b); white-space: nowrap;">{{{{ issue.name }}}}</code>
            <span class="meta" style="font-size: 11px; text-transform: uppercase; letter-spacing: 0.04em;">{{{{ issue.status }}}}</span>
            <span style="color: var(--text-primary, #2f363c); flex: 1; min-width: 140px;">{{{{ issue.hint }}}}</span>
            <!-- The command chip carries an ARBITRARY-LENGTH command (e.g.
                 `trinity-local lens --only-distill`). With white-space:nowrap and
                 no max-width it became a single unbreakable flex item WIDER than a
                 narrow card (320px side panel) — so even inside a flex-wrap row it
                 pushed ~14px past the card's right edge and the whole sandbox doc
                 scrolled sideways (~13px). Cap it to the row width and let the
                 command WRAP (it stays fully readable, just on two lines) so a long
                 command can never overflow the container. -->
            <button v-if="issue.command"
                    type="button"
                    @click="copyHealthCommand(issue)"
                    :title="'Copy: ' + issue.command"
                    style="font-family: ui-monospace, monospace; font-size: 12px; color: var(--text-primary, #2f363c); background: var(--bg-base, #eaecef); border: 1px solid var(--border, #dde1e6); border-radius: 4px; padding: 4px 10px; cursor: pointer; white-space: normal; overflow-wrap: anywhere; max-width: 100%; text-align: left;">
              <span v-if="copiedKey === 'health-' + issue.name + issue.status">✓ Copied</span>
              <span v-else>{{{{ issue.command }}}}</span>
            </button>
            <a v-if="issue.href"
               :href="issue.href"
               style="font-size: 12px; color: var(--accent-deep, #34666b); text-decoration: none; padding: 4px 10px; border: 1px solid var(--border, #dde1e6); border-radius: 4px; white-space: nowrap;">
              Inspect →
            </a>
          </li>
        </ul>
      ...
      <section
        ...
        role="status"
        aria-live="polite"
        style="border-left: 3px solid #4f9095; background: rgba(79, 144, 149, 0.06);"
      >
        <!-- inExtensionPanel guard (sibling of the proactive card's at ~L1268):
             this REACTIVE banner used to tell EVERY failed-dispatch user to
             "Install the Trinity browser extension" + "load the browser-extension/
             folder" — a founder-class contradiction when it fires inside the
             extension's OWN side panel (you're plainly already in it). The
             proactive card got the inExtensionPanel reframe; this reactive twin
             was missed. In the panel a 'no-route' failure means the extension is
             present but the host isn't wired — so it collapses to the same
             register-the-host remediation as native-host-unavailable. The file://
             launchpad (!inExtensionPanel) keeps the full install pitch. -->
        <div class="eyebrow" style="color: #34666b;">
          <span v-if="dispatchBannerReason === 'native-host-unavailable' || inExtensionPanel">Extension installed, host not registered</span>
        ...
        <h2 style="margin-top: 4px; font-size: 18px;">
          ...
          <span v-else-if="inExtensionPanel">
            The extension is installed — its capture host just isn't wired up yet
          </span>
          <span v-else>
        ...
        <p class="meta" style="margin-top: 8px;" v-else-if="inExtensionPanel">
          The extension is installed (you're in its side panel) — the dispatch
          had no route because the local helper isn't registered yet. Run
          <code>trinity-local install-extension --extension-id &lt;ID&gt;</code>
          (the popup shows your ID) and reload.
        </p>
        <p class="meta" style="margin-top: 8px;" v-else>
      ...
      <!-- Portal deep-link bounce ack. A memory-viewer link (→ topology /
           picks.json / a lens chip) clicked in the side panel can't self-nav to
           memory.html (not in the sandbox), so the runtime opens the FULL dashboard
           in a browser window. open-launchpad's success is silent, so without this
           the click read as a dead no-op. This transient note (set by
           showPortalNotice, role=status so it's announced too) tells the user the
           click landed AND where it went. Visible in BOTH views (the chips live on
           /stats but the note is global). -->
      <section
        class="card portal-open-notice"
        v-if="portalNotice"
        role="status"
        aria-live="polite"
        style="border-left: 3px solid var(--accent, #4f9095); display: flex; align-items: flex-start; gap: 10px;"
      >
        <p class="meta" style="margin: 0; flex: 1;">{{{{ portalNotice }}}}</p>
        <a href="#" @click.prevent="dismissPortalNotice" class="meta" style="white-space: nowrap; flex-shrink: 0;" aria-label="Dismiss">Dismiss</a>
      </section>
      ...
      <section class="card stats-card">
        ...
        <div class="memory-links" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); gap: 8px; margin-top: 16px;">
          <a class="memory-chip" href="../portal_pages/memory.html?file=core.md"
            ...
            <code style="color: var(--accent-deep, #34666b); font-size: 13px;">core.md</code>
          ...
          <a class="memory-chip" href="../portal_pages/memory.html?file=lens.md"
             style="display: block; padding: 10px 12px; border: 1px solid var(--border); border-radius: 6px; text-decoration: none; color: inherit;">
            <code style="color: var(--accent-deep, #34666b); font-size: 13px;">lens.md</code>
          ...
          <a class="memory-chip" href="../portal_pages/memory.html?file=topics.json"
             style="display: block; padding: 10px 12px; border: 1px solid var(--border); border-radius: 6px; text-decoration: none; color: inherit;">
            <code style="color: var(--accent-deep, #34666b); font-size: 13px;">topics.json</code>
          ...
          <a class="memory-chip" href="../portal_pages/memory.html?file=vocabulary.md"
             style="display: block; padding: 10px 12px; border: 1px solid var(--border); border-radius: 6px; text-decoration: none; color: inherit;">
            <code style="color: var(--accent-deep, #34666b); font-size: 13px;">vocabulary.md</code>
      ...
      <!-- Surface 33 (v1.6) — Browser capture activity. Shows per-
           ...
           collapse. -->
      <details class="demoted-card-wrapper"{details_open}
               :class="{{ 'stats-card': !pageData.browserCapture.stale }}"
               v-if="pageData.browserCapture" style="margin-bottom: 18px;">
        <summary style="cursor: pointer; padding: 10px 14px; background: rgba(0,0,0,0.025); border-radius: 6px; font-size: 13px; display: flex; align-items: center; gap: 12px;"
                 :style="pageData.browserCapture.has_data && pageData.browserCapture.stale ? 'background: rgba(179, 38, 30, 0.07);' : ''">
          <span class="eyebrow" style="margin: 0; padding: 0;">Browser capture</span>
          ...
          <span v-else-if="pageData.browserCapture.has_data && pageData.browserCapture.stale" style="font-weight: 600; color: #b3261e;">
            ⚠ no captures in 24h · last {{{{ pageData.browserCapture.last_capture_ago_human }}}} ago — expand to repair
          </span>
          <!-- In the side panel the user is ALREADY inside the installed
               extension, so "Extension not installed" is a flat contradiction
               (the cross-bootstrap card's inExtensionPanel reframe at ~L1622 —
               this empty-capture twin was the missed sibling). What's actually
               missing there is CAPTURED web chats, not the extension; reframe to
               the real next step (browse your web chats). The file:// launchpad,
               where the extension may genuinely be absent, keeps the install copy. -->
          <span v-else-if="inExtensionPanel" style="font-weight: 500;">
            No web chats captured yet — browse Claude / ChatGPT / Gemini to start
          </span>
      ...
      <!-- View-class is MODE-CONDITIONAL (UX sweep iter 82 — the same #273
           "don't let degradation be silent" trigger Iter-81 fixed on the
           embedder card, found re-buried here by the home/stats split). This
           card's whole reason to exist (its docstring: "makes silent capture
           breakage VISIBLE") splits into two states:
             - HEALTHY (stale=false) = "N conversations captured · last 24h" → pure
               operational analytics. `stats-card` → /stats only; the simple home
               stays a clean council surface.
             - STALE (stale=true) = "⚠ No new captures in 24h — the provider may
               have refactored their streaming API" + the self-healing Repair CTA.
               That is the SILENT capture-breakage the card was built to surface (the
               user's corpus silently stopped growing). It is a REGRESSION, not
               analytics, so it must show on HOME, not hide behind a /stats click whose
               only home trace is NOTHING (the home shows no capture signal at all, so
               a broken capture reads identically to a healthy one). Untagged →
               visible on BOTH views. Mirrors the embedder card's reinstall-libs mode. -->
      <section
        :class="'card browser-capture-card ' + (pageData.browserCapture && pageData.browserCapture.stale ? 'browser-capture-stale-card' : 'stats-card')"
        v-if="pageData.browserCapture && pageData.browserCapture.has_data"
        :style="pageData.browserCapture.stale ? 'border-left: 3px solid #b3261e; background: rgba(179, 38, 30, 0.04);' : 'border-left: 3px solid #7fa0ad; background: rgba(43, 80, 112, 0.04);'">
        <div class="eyebrow" :style="pageData.browserCapture.stale ? 'color: #b3261e;' : 'color: #34666b;'">
          <span v-if="pageData.browserCapture.stale">Capture stalled · corpus not growing</span>
          <span v-else>Browser capture<span v-if="pageData.browserCapture.captured_24h > 0"> · last 24h</span></span>
        </div>
        <h2 style="margin-top: 4px; font-size: 18px;">
          ...
          <span style="float: right; font-variant-numeric: tabular-nums;"
                :style="pageData.browserCapture.stale ? 'color: #b3261e;' : 'color: #34666b;'">
        ...
        <ul style="list-style: none; padding: 0; margin: 12px 0 0; display: flex; flex-direction: column; gap: 4px; font-variant-numeric: tabular-nums; font-size: 13px;">
          <li v-for="row in pageData.browserCapture.providers" :key="row.provider"
              class="bc-provider-row"
              style="display: grid; gap: 8px; align-items: center;">
            <!-- Brand label, NOT the raw capture-source directory slug. The
                 conversations/ dir names are the web-era capture slugs
                 (claude / chatgpt / gemini); formatProviderLabel folds them
                 through normalizeProviderSlug (chatgpt→codex→GPT,
                 gemini→antigravity→Gemini, claude→Claude) so this row reads
                 the SAME brand trio (#275) as every other launchpad surface
                 instead of leaking a lowercase "chatgpt" / "gemini". -->
            <span>{{{{ formatProviderLabel(row.provider) }}}}</span>
            <span class="meta">{{{{ row.count }}}}</span>
            <span style="position: relative; height: 6px; background: rgba(0,0,0,0.06); border-radius: 3px;">
              <span :style="'position: absolute; left: 0; top: 0; bottom: 0; width: ' + (row.count / pageData.browserCapture.total_captured * 100) + '%; background: ' + (pageData.browserCapture.stale ? '#b3261e' : '#7fa0ad') + '; border-radius: 3px;'"></span>
            </span>
            ...
            <span v-if="row.missing_count > 0"
                  class="meta"
                  style="text-align: right; color: #34666b; font-size: 12px;"
        ...
        <p class="meta" style="margin-top: 12px; color: #b3261e;" v-else>
          ⚠ No new captures in the last 24h (last was {{{{ pageData.browserCapture.last_capture_ago_human }}}} ago) —
          the provider may have refactored their streaming API. Trinity can heal itself: dispatch the
          self-healing council to diagnose the break and propose an extension patch (no HAR needed).
        </p>
        <!-- Self-healing flagship (#147) surfaced at the SYMPTOM. The "Repair
             extension" button used to live ONLY in the memory-health card's
             eyebrow, which renders on an UNRELATED trigger (a stale cognitive
             memory file) — so a user staring at this broken-capture card had no
             path to the auto-repair flow and was sent to chrome://extensions to
             debug by hand. Same repairExtension() action + repairExtensionStatus
             state (one shared petite-vue root), so the two surfaces can't diverge. -->
        <div v-if="pageData.browserCapture.stale" style="margin-top: 10px; display: flex; align-items: center; gap: 10px; flex-wrap: wrap;">
          <button type="button"
                  class="suggestion-chip"
                  style="padding: 5px 12px; font-size: 13px; cursor: pointer;"
                  :disabled="repairExtensionStatus === 'running'"
                  title="Dispatches a full council (all your providers) on your own subscription"
                  @click="repairExtension">
            <span v-if="repairExtensionStatus === 'running'">
          ...
          <span class="meta" style="font-size: 12px;">
            Or check the service-worker console at <code>chrome://extensions</code> —
            details in <code>browser-extension/README.md</code>.
          </span>
        </div>
        <!-- Co-located repair failure (#242(a) class): keep the WHY on this card
             rather than leaking it to the home-view council ribbon. -->
        <p class="meta" v-if="repairExtensionStatus === 'failed' && repairExtensionError"
           style="margin-top: 8px; color: var(--warning-text, #79591b); font-size: 12px;">
          ⚠ Repair extension couldn't dispatch: {{{{ repairExtensionError }}}}. Try again, or run <code>trinity-local extension repair --auto</code> in your terminal.
        </p>
      </section>

      <section class="card browser-capture-card stats-card"
               v-if="pageData.browserCapture && !pageData.browserCapture.has_data"
               style="border-left: 3px dashed rgba(43, 80, 112, 0.4); background: rgba(43, 80, 112, 0.02);">
        <div class="eyebrow" style="color: #34666b;">Browser capture</div>
        <!-- In the side panel the extension is plainly INSTALLED (you're in it),
             so "install the v1.6 browser extension" + a copy-the-install-command
             CTA is a contradiction + a dead affordance (the real gap is captured
             CHATS, not the extension). Reframe the empty state to its true next
             step there: browse your web chats so capture has something to record.
             The file:// launchpad (extension may be absent) keeps the install pitch. -->
        <h2 style="margin-top: 4px; font-size: 18px;" v-if="inExtensionPanel">
          No web chats captured yet — open Claude / ChatGPT / Gemini to start
        </h2>
        <h2 style="margin-top: 4px; font-size: 18px;" v-else>
          Capture every Claude / ChatGPT / Gemini conversation automatically
        </h2>
        <p class="meta" style="margin-top: 4px;" v-if="inExtensionPanel">
          The extension is installed and capturing — it just hasn't seen a web chat yet.
          Visit claude.ai, chatgpt.com, or gemini.google.com and send a message; each one
          lands in <code>~/.trinity/conversations/</code> automatically and flows into your
          cortex / lens / picks. Nothing else to run.
        </p>
        <p class="meta" style="margin-top: 4px;" v-else>
          ...
          you send on claude.ai, chatgpt.com, or gemini.google.com lands in <code>~/.trinity/conversations/</code>
          and flows into your cortex / lens / picks.
        </p>
        <div class="code-copy-wrap" v-if="!inExtensionPanel">
          <pre class="md-code-block"><code>{{{{ pageData.browserCapture.install_command }}}}</code></pre>
          <button type="button" class="copy-badge" title="Copy command" aria-label="Copy command"
                  @click="copyCodeBlock($event, 'copy-extension-install')">
            <span v-if="copiedKey === 'copy-extension-install'">✓ Copied</span>
            <span v-else>Copy</span>
          </button>
        </div>
        <p class="meta" style="margin-top: 8px;" v-if="!inExtensionPanel">
      ...
      <!-- #148 bulk Takeout import UI — sibling to browser-capture
           ...
           user's machine). -->
      <section class="card import-export-card stats-card"
               style="border-left: 3px solid rgba(43, 80, 112, 0.3);">
        <!-- task #148 — internal ref in the comment, not the rendered eyebrow. -->
        <div class="eyebrow" style="color: #34666b;">Bulk import</div>
        <h2 style="margin-top: 4px; font-size: 18px;">
        ...
        <!-- min-width:0 lets the flex input shrink below its intrinsic
             ~186px text-input min-content (the CSS default min-width:auto
             pins it, so at <=360px the row couldn't shrink and the Probe
             button was shoved 0.8px past the 320px viewport — the same
             child-escapes-card overflow class as the #252 timeline card,
             iter 92). flex-wrap is the sub-280 belt-and-suspenders so the
             button wraps below the input instead of spilling off-screen. -->
        <div style="display: flex; flex-wrap: wrap; gap: 8px; margin-bottom: 10px;">
          <input v-model="importPath"
                 @input="onImportPathInput"
                 type="text"
                 aria-label="Path to a Takeout zip or exports folder to import"
                 placeholder="/Users/you/Downloads/Takeout"
                 style="flex: 1 1 120px; min-width: 0; padding: 8px 10px; font-family: ui-monospace, monospace; font-size: 13px; border: 1px solid var(--border, #dde1e6); border-radius: 4px; background: var(--bg-base, #eaecef);" />
          <button type="button"
                  @click="probeImportPath"
                  :disabled="!importPath || importStatus === 'probing' || importStatus === 'importing'"
                  class="suggestion-chip"
                  style="flex-shrink: 0; padding: 6px 14px; cursor: pointer; font-size: 13px;">
            <span v-if="importStatus === 'probing'">Probing…</span>
            <span v-else>Probe</span>
          </button>
        </div>
        <div v-if="importProbeResult && !importProbeResult.error"
             style="padding: 10px 12px; background: rgba(45, 106, 79, 0.06); border-left: 3px solid var(--success, #4f9095); border-radius: 0 4px 4px 0; margin-bottom: 10px; font-size: 13px;">
          <strong style="color: var(--success-text, #2d6a4f);">✓ Detected {{{{ importProbeResult.detected.length }}}} export(s)</strong>
          <ul style="list-style: none; padding: 8px 0 0; margin: 0; font-size: 12px;">
            <li v-for="(e, i) in importProbeResult.detected" :key="'imp-' + i"
                style="font-family: ui-monospace, monospace; color: var(--text-secondary, #5b646b);">
              <span style="color: var(--success-text, #2d6a4f);">{{{{ e.source }}}}</span> · {{{{ e.hint }}}}
            </li>
          </ul>
          <button type="button"
                  @click="confirmImport"
        ...
        <div v-if="importProbeResult && importProbeResult.error"
             style="padding: 10px 12px; background: rgba(79, 144, 149, 0.06); border-left: 3px solid var(--warning, #bd9658); border-radius: 0 4px 4px 0; margin-bottom: 10px; font-size: 13px; color: var(--warning-text, #79591b); overflow-wrap: anywhere; word-break: break-word; min-width: 0;">
          ⚠ {{{{ importProbeResult.error }}}}
          <span v-if="importProbeResult.hint" style="display: block; margin-top: 4px; font-size: 12px; opacity: 0.85; color: var(--text-primary, #2f363c);">
      ...
      <p class="meta" style="text-align: center; margin-top: 24px; font-size: 12px;" v-if="debugMode">
