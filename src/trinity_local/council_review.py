...
def render_live_council_page() -> str:
    ...
    page_data = {
        ...
        # Stop-council and failed-council retry clicks through the Chrome
        # extension. Without this, dispatcher.state='absent' and those
        # buttons can't find a path to the CLI.
  ...
  <div id="live-council-app" v-scope="LiveCouncilApp(pageData)" v-cloak @vue:mounted="init">
    ...
    <main>
            ...
            <div v-if="routingLabelFor(seg).disagreed_claims && routingLabelFor(seg).disagreed_claims.length && !isSoloFor(seg)">
              ...
              <ul>
                <li v-for="d in routingLabelFor(seg).disagreed_claims">
                  ...
                  <!-- The prosecutorial chairman's adjudication of the split:
                       which side survives the other members' evidence, or
                       "unresolved". This is the verdict the Contested section
                       exists to produce, so it renders ABOVE why_matters. -->
                  <div v-if="d.resolution" class="meta"><strong>Verdict:</strong> {{{{ d.resolution }}}}</div>
                  <div v-if="d.why_matters" class="meta">{{{{ d.why_matters }}}}</div>
                </li>
              </ul>
            </div>
            <!-- facets: the free-form dimensions the chairman says ACTUALLY
                 discriminated between the answers, each with the member that
                 won it (prosecutor-era field, 2026-07-24; absent on older
                 councils — v-if degrades this block to nothing). -->
            <div v-if="routingLabelFor(seg).facets && routingLabelFor(seg).facets.length && !isSoloFor(seg)">
              <strong>What discriminated</strong>
              <ul>
                <li v-for="f in routingLabelFor(seg).facets">
                  <span>{{{{ f.name }}}}</span>
                  <span v-if="f.winner" class="meta"> — {{{{ formatProviders([f.winner]) }}}}</span>
                  <div v-if="f.basis" class="meta">{{{{ f.basis }}}}</div>
                </li>
              </ul>
            </div>
          ...
          <div :class="memberRowsFor(seg).length === 3 ? 'answers-grid answers-grid-three' : 'answers-grid'">
            ...
            >
              <div class="provider-status-header">
                ...
                <!-- Quote ↓ removed 2026-07-24 with the "Continue the thread"
                     composer it fed: the council-iterate CLI verb (and the
                     run_consensus_round engine behind it) were removed, so the
                     composer had nothing to dispatch to. Its own 2026-06-19 rule
                     — "no composer → no Quote button" — now applies permanently. -->
              </div>
      ...
      <!-- chainError banner. Still load-bearing after the "Continue the
           thread" composer was removed (2026-07-24): Stop council and the
           failed-council "Try again" retry both surface dispatch failures
           here. -->
      ...
      <!-- The "Continue the thread" composer (Continue / Auto-chain / Refine)
           was removed 2026-07-24 together with the council-iterate CLI verb and
           the run_consensus_round engine. Every button here dispatched
           `kind: 'council-iterate'` through the extension, which capture_host
           spawned DETACHED with stdio to /dev/null — so once the verb was gone
           the clicks failed silently and this page polled forever for a status
           no process would ever write. Historical multi-round threads still
           RENDER (the segment list above reads chain_root_id/round_number off
           disk); they just cannot be extended. -->
    </main>
  </div>
  ...
  <script>
    ...
    function LiveCouncilApp(pageData) {{
      ...
      return {{
        ...
        // COPY FIX 2026-08-01: was 'Could not start next round'. Rounds cannot be
        // started any more — the Continue / Auto-chain / Refine composer was deleted
        // 2026-07-24 with the council-iterate verb. The only user action that still
        // reaches this banner is retryFailedCouncil(), whose own comment says it is
        // "NOT a chain-iterate" but which nevertheless set this heading, so a failed
        // RETRY told the user a ROUND failed. Phrasing is now action-neutral; the two
        // sites that used to re-assert the round wording set the same constant.
        chainErrorHeading: 'Could not dispatch',
        ...
        init() {{
          ...
          // hint on load — before the user hits the rejection (founder report
          // 2026-05-31). A reload, not a reinstall, fixes it.
          //
          // COPY FIX 2026-08-01: this message used to read "Refine / Continue
          // won't work until you reload it". Refine and Continue were DELETED on
          // 2026-07-24 with the council-iterate verb (see the composer comment
          // above), so the warning was naming two controls the user cannot find,
          // for a page whose only remaining dispatch is Retry. The stale state is
          // still real and still reachable — retryFailedCouncil() sets chainError
          // — so the warning stays; only the dead control names go.
          ...
          if (_disp) {{
            ...
            const _onState = (st) => {{
              if (st === 'stale' && !this.chainError) {{
                ...
                this.chainError = "Dispatching from this page won't work until you reload it at chrome://extensions.";
                ...
                this.chainError = '';
                this.chainErrorHeading = 'Could not dispatch';
              }}
            }};
        ...
        _loadOutcomeIntoSegment(seg, councilId) {{
          loadOutcomeScript(councilId, (outcome) => {{
            ...
            // this round, back when rounds could be started (the composer
            // and council-iterate were removed 2026-07-24). Historical
            // rounds still carry it, so it still renders.
        ...
        retryFailedCouncil() {{
          ...
          this.chainError = '';
          this.chainErrorHeading = 'Could not retry the council';
          this.chainBusy = true;
